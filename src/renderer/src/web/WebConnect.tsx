...
export default function WebConnect({
  ...
  const [name, setName] = useState(existingEnvironment?.name ?? 'Argus Server')
  ...
  const connect = async (): Promise<void> => {
    setError(null)
    if (!parsedOffer) {
      setError('Enter a valid Argus pairing URL or pairing code.')
      return
    }
    ...
    if (isMixedContentWebSocket(parsedOffer.endpoint)) {
      setError(
        'This HTTPS page cannot connect to a plain ws:// Argus server. Open the web client over HTTP or pair with a wss:// endpoint.'
      )
      return
    }
    setConnecting(true)
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-base font-semibold leading-6">
              {translate('auto.web.WebConnect.e3bcd082ac', 'Connect to Argus')}
            </h1>
            <p className="mt-1 text-sm leading-5 text-muted-foreground">
              {translate(
                'auto.web.WebConnect.3affe7de3a',
                'Paste a pairing URL from an Argus server that this browser can reach.'
              )}
            </p>
          </div>
        </div>
