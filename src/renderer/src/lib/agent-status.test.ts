...
describe('detectAgentStatusFromTitle', () => {
  ...
  // Why: cursor-agent's native title stays "Cursor Agent" all turn, so Argus synthesizes decorated titles for the spinner/unread pipeline.
...
describe('getAgentLabel', () => {
  ...
  it('treats Claude Code prefixed task titles as Claude even when they mention another CLI', () => {
    ...
    expect(getAgentLabel('. Compare Opencode Vs Argus')).toBe('Claude Code')
  ...
  it('labels Cursor by identity, not a bare "cursor" token in another agent title', () => {
    ...
    expect(getAgentLabel('Terminal Cursor and Argus slows down')).toBeNull()
  })
})
