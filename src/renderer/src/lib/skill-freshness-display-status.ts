...
export function getSkillFreshnessDisplayStatus(
  ...
  for (const installation of inventory?.installations ?? []) {
    ...
    // drift Argus has no way to fix. Skipped before `hasPlacement` so a repo-only skill
    // reports presence, not a freshness claim about a copy Argus does not manage.
  ...
  // fail-closed — but only for skills Argus actually found somewhere. Flagging a skill
