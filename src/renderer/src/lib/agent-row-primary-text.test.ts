...
describe('getAgentRowPrimaryText', () => {
  it('prefers orchestration display name over the raw hook prompt', () => {
    expect(
      getAgentRowPrimaryText({
        prompt: 'You are working inside Argus, a multi-agent IDE.',
        orchestration: {
          taskId: 'task-1',
          dispatchId: 'ctx-1',
  ...
  it('ignores sticky orchestration labels that belong to a different task id', () => {
    expect(
      getAgentRowPrimaryText({
        prompt: `You are working inside Argus, a multi-agent IDE. You are a dispatched worker.
...
${taskBody}`
    ...
    expect(preview).not.toContain('You are working inside Argus')
...
Implement the detailed worker instructions that should not stay as the final label`
  ...
  it('does not surface preamble boilerplate when the TASK marker is absent', () => {
    expect(
      getAgentRowPrimaryText({
        prompt:
          'You are working inside Argus, a multi-agent IDE. You are a dispatched worker. Your task ID is: task_no_body CLI noise only'
      })
    ).toBe('')
  })
  ...
  // mentions `=== TASK ===` wins over Argus's real separator.
  it('ignores adversarial === TASK === text in raw multi-line base-drift subjects', () => {
    expect(
      getAgentRowPrimaryText({
        prompt: [
          'You are working inside Argus, a multi-agent IDE. You are a dispatched worker.',
  ...
  it('still reads the task body from a normalized single-line compact prompt', () => {
    expect(
      getAgentRowPrimaryText({
        prompt:
          'You are working inside Argus, a multi-agent IDE. Your task ID is: task_inline === TASK === Compact body preview'
      })
...
body`
  ...
  it('extracts the id from a raw multi-line prompt', () => {
    expect(
      getOrcaDispatchTaskId(
        `You are working inside Argus, a multi-agent IDE.\nYour task ID is: task_1\n\n=== TASK ===\nbody`
      )
    ).toBe('task_1')
  })
})
...
describe('isOrcaDispatchPrompt / getAgentRowGeneratedTitleText', () => {
  it('treats leading whitespace as still a dispatch preamble', () => {
    expect(
      isOrcaDispatchPrompt('  You are working inside Argus, a multi-agent IDE. Worker task')
    ).toBe(true)
  })
