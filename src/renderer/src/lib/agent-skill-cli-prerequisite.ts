...
export const AGENT_SKILL_CLI_PREREQUISITE_NOTICE =
  'Before opening setup, Argus may show a system prompt to register the Argus CLI command on PATH.'

export const CLI_PREREQUISITE_REGISTRATION_TOAST = 'Argus needs to register its CLI on PATH.'
export const CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION =
  'Approve the system prompt so skill setup can use the Argus CLI command.'
    ...
    toast.error(
      error instanceof Error
        ...
        : translate(
            ...
            'Failed to register the Argus CLI in PATH.'
          )
    )
    return null
  }
...
function showCliPrerequisiteWarning(status: CliInstallStatus): void {
  if (!status.supported) {
    toast.warning(
      translate(
        ...
        'Argus CLI registration is unavailable'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.state !== 'installed') {
    toast.warning(
      translate(
        ...
        'Argus CLI registration needs attention'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === null) {
    toast.warning(
      translate(
        ...
        'Argus could not check your Windows user PATH'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === false) {
    ...
    // Argus affordances when that shell can resolve the Argus CLI command.
    toast.warning(
      translate(
        'auto.lib.agent.skill.cli.prerequisite.79371593b0',
        'Argus CLI is not visible on PATH yet'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Restart your shell or add the Argus CLI directory to PATH before setup.'
          )
      }
    )
  }
