...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `Argus pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior Argus session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest Argus status hints:', ...statusHints] : []),
    '',
