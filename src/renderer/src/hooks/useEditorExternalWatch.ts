...
function scheduleChangedOnDiskMark(
  ...
  const recentSelfWrite = getRecentSelfWrite(absolutePath, target.runtimeEnvironmentId)
  // Why: the fs event may be the echo of Argus's own save — verify disk really differs from our last write before showing a "changed on disk" banner.
...
function scheduleSelfWriteAwareExternalReload(
  ...
  // Why: a self-write stamp only proves recent change; compare disk content so we suppress only Argus's own echo, not a newer agent write in the same TTL.
  void readFileForEchoVerification({
