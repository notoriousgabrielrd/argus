 ...
 * Argus (#9756). Users who never see one never see this.
 */
    ...
    return subscribeToMacosTccPromptNotice(window.api?.macosTccPrompts, (_, acknowledge) => {
      toast.warning(
        translate(
          ...
          'Seeing “Argus would like to access…” prompts?'
        ),
        {
          description: translate(
            'auto.hooks.useMacosTccPromptNotice.description',
            'Permission messages from macOS may appear when an agent or terminal tool running in Argus attempts to access protected files. Grant Full Disk Access in Settings to reduce these prompts.'
          ),
          duration: Infinity,
