...
/**
 * Open a local AI Vault session log inside Argus as a permanent, read-only editor
 * tab (or activate an existing tab without reducing its authority). Reuses
 * Argus's external-file authorize + `openFile` pipeline; it never grants write
...
export async function openAiVaultSessionLogInOrca(session: AiVaultLogSession): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      // trust gesture. Reuses Argus's existing external R/W open grant.
