...
describe('buildAiVaultProjectContext', () => {
  it('uses durable worktree project ids before repo fallback', () => {
    ...
    const project = makeProject({ id: 'project-1', displayName: 'Canonical Argus' })
    ...
    expect(context.sessionProjectById.get(baseSession.id)).toMatchObject({
      ...
      label: 'Canonical Argus'
    })
  })

  it('normalizes compatibility project ids to repo keys', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Argus', path: '/Users/ada/orca' })
    const worktree = makeWorktree({
      id: 'wt-1',
      repoId: repo.id,
    ...
    const context = buildAiVaultProjectContext({
      ...
      worktrees: [worktree],
      projectHostSetupProjection: makeProjection({
        projects: [makeProject({ id: 'repo:repo-1', displayName: 'Compatibility Argus' })],
    ...
    expect(context.projectLabelByKey.get('repo:repo-1')).toBe('Argus')
  })
  ...
  it('inherits setup project ids for legacy worktrees without project metadata', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Argus Repo', path: '/repo/orca' })
    const worktree = makeWorktree({
      id: 'wt-legacy',
      repoId: repo.id,
      path: '/repo/orca'
    ...
    const context = buildAiVaultProjectContext({
      ...
      worktrees: [worktree],
      projectHostSetupProjection: makeProjection({
        projects: [makeProject({ id: 'github:stablyai/orca', displayName: 'Canonical Argus' })],
        setups: [
          makeSetup({
            repoId: repo.id,
  ...
  it('maps null cwd sessions to unknown', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Argus', path: '/repo' })
