...
describe('filterAiVaultSessions', () => {
  ...
  it('filters project scope by the resolved active project key', () => {
    ...
    const sessionProjectById = new Map([
      [projectSession.id, { kind: 'repo' as const, key: 'project:orca', label: 'Argus' }],
  ...
  it('matches repo: queries against resolved project labels before folder fallback', () => {
    const sessionProjectById = new Map([
      [baseSession.id, { kind: 'repo' as const, key: 'project:orca', label: 'Canonical Argus' }]
    ])
    const projectLabelByKey = new Map([['project:orca', 'Canonical Argus']])
  ...
  it('groups sibling worktree sessions by project label when resolved', () => {
    ...
    const sessionProjectById = new Map(
      sessions.map((session) => [
        session.id,
        { kind: 'repo' as const, key: 'project:orca', label: 'Argus' }
      ])
    )
    const projectLabelByKey = new Map([['project:orca', 'Canonical Argus']])
    ...
    ).toEqual([{ key: 'project:orca', label: 'Canonical Argus', sessions }])
  })
