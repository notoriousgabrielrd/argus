...
describe('checks panel hosted review modifier hint destination', () => {
  it('names the system browser when a plain click already opens in Argus', () => {
    expect(resolveChecksPanelHostedReviewModifierDestination({ openLinksInApp: true }, true)).toBe(
      'system-browser'
    )
  })

  // Why: inverting is inert while links already open in Argus — both meanings of the
  // modifier land on the system browser, so checking inverts first would misname it.
  it('names the system browser when inverting is on and links already open in Argus', () => {
    expect(
      resolveChecksPanelHostedReviewModifierDestination(
  ...
  // and the only way to reach Argus from this button is undiscoverable.
  it('names Argus when inverting is on and links open externally', () => {
    expect(
      resolveChecksPanelHostedReviewModifierDestination(
  ...
  // Why: openHttpLink refuses to route a remote-owned link into Argus, and openLinksInApp
  ...
  // not suppress a hint for a click that still reaches Argus.
  ...
  // Why: openHttpLink gates routing to Argus on a worktree id, so without one the
