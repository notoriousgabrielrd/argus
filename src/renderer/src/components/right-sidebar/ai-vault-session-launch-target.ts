...
export function activateAiVaultResumeWorkspace(workspaceId: string): void {
  // Why: the resumed tab already landed in tabsByWorktree[sentinel] — just
  // switch the main area to the global terminal page, same as jumpToOriginalPane.
  if (workspaceId === FLOATING_TERMINAL_WORKTREE_ID) {
    useAppStore.getState().openGlobalTerminalPage()
    return
  }
