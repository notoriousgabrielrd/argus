...
describe('LinuxPackageInstallRecoveryCard copy', () => {
  it('leads with the recovery copy and the three dedicated actions', () => {
    ...
    expect(
      screen.getByText(/a system terminal on the computer where Argus is installed/)
    ).toBeTruthy()
    expect(screen.getByText(/quit and reopen Argus to run the new version/)).toBeTruthy()
...
describe('LinuxPackageInstallRecoveryCard details', () => {
  ...
  it('scopes the trust note to the moment the command is built', () => {
    ...
    const detail = screen.getByText(/Argus checks the downloaded file against the release metadata/)
    expect(detail.textContent).toContain('at the moment it builds this command')
    expect(detail.textContent).toContain(
      'The system package itself is not signature-checked, and Argus cannot vouch for the file ' +
        'after that point.'
    )
  })
