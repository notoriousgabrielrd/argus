...
describe('provider usage error copy', () => {
  ...
  it('shows the exact Grok CLI recovery flow for an expired refreshable session (#8497)', () => {
    const grok = provider({
      provider: 'grok',
      error:
        'Grok sign-in expired — run grok on the computer running Argus; sign in if prompted. No chat message is needed.',
      usageMetadata: {
  ...
  it('shows the exact Kimi CLI recovery flow for an expired read-only session', () => {
    const kimi = provider({
      provider: 'kimi',
      error: 'Kimi session expired — run kimi on the computer running Argus, then retry usage.',
      usageMetadata: {
        failureKind: 'delegated-refresh-required',
        source: 'oauth'
      }
    ...
    expect(getProviderUsageErrorMessage(kimi)).toBe(
      'Run kimi in a terminal on the computer running Argus and wait for it to start, then retry usage.'
    )
  })
