...
function eligibleInventory(): SkillFreshnessInventory {
  return {
    schemaVersion: 1,
    installations: [placement('argus-cli')],
    eligibleUpdateNames: ['argus-cli'],
    scanIssues: [],
    scannedAt: 1
  }
}
...
describe('SkillFreshnessUpdateDialog', () => {
  ...
  it('starts a background run with the eligible names', async () => {
    ...
    expect(skillsApi.startUpdateRun).toHaveBeenCalledWith(['argus-cli'])
  })

  it('shows indeterminate progress and says the run survives closing', async () => {
    await renderDialog()
    await openViaRequest()
    await emitRun({ state: 'running', names: ['argus-cli'], startedAt: 1, output: '' })
    ...
    expect(
      container?.querySelector('[data-skill-row="argus-cli"]')?.getAttribute('data-state-label')
    ).toBe('pending')
  })
  ...
  it('attributes failures to the names the re-scan says are still outdated', async () => {
    mocks.inventory = {
      schemaVersion: 1,
      installations: [placement('argus-cli'), placement('orchestration')],
      eligibleUpdateNames: ['argus-cli', 'orchestration'],
      scanIssues: [],
      scannedAt: 1
    }
    await renderDialog()
  ...
  it('keeps the same row elements across the whole run instead of swapping layouts', async () => {
    await renderDialog()
    await openViaRequest()
    const before = container?.querySelector('[data-skill-row="argus-cli"]')
    expect(before?.getAttribute('data-state-label')).toBe('available')

    await emitRun({ state: 'running', names: ['argus-cli'], startedAt: 1, output: '' })
    const during = container?.querySelector('[data-skill-row="argus-cli"]')
    ...
    mocks.inventory = {
      schemaVersion: 1,
      installations: [placement('argus-cli', { status: 'current', installedReleaseRevision: 2 })],
      eligibleUpdateNames: [],
      scanIssues: [],
      scannedAt: 5
    }
    await emitRun({ state: 'success', names: ['argus-cli'], finishedAt: 2, output: 'done' })
    await rerender()
    const after = container?.querySelector('[data-skill-row="argus-cli"]')
  ...
  it('collapses each skill’s locations behind its own disclosure', async () => {
    ...
    const row = container?.querySelector('[data-skill-row="argus-cli"]')
  ...
  it('names the skill in the stale-record remedy so the command is runnable as-is', async () => {
    ...
    expect(container?.textContent).toContain(
      'npx skills add https://github.com/notoriousgabrielrd/argus --skill orchestration --global'
    )
  })
  ...
  it('surfaces a scan error instead of a stale summary', async () => {
    ...
    expect(container?.querySelector('[data-skill-row="argus-cli"]')).not.toBeNull()
    ...
    expect(container?.querySelector('[data-skill-row="argus-cli"]')).toBeNull()
    expect(findButton('Update 1 skill')).toBeUndefined()
  })
  ...
  it('shows incomplete plugin coverage without presenting a fabricated skill copy', async () => {
    mocks.inventory = {
      schemaVersion: 1,
      installations: [
        placement('argus-cli', { status: 'current', observedPackageDigest: 'current' })
      ],
      eligibleUpdateNames: [],
      scanIssues: [
        {
    ...
    expect(container?.textContent).not.toContain(
      '/home/.codex/plugins/cache/vendor/locked/argus-cli'
    )
  })
