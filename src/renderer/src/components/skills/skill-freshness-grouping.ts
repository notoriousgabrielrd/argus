 ...
 * wrong in a way the update cannot fix — an edited copy, one Argus could not read.
 *
...
export function groupSkillFreshness(
  ...
  for (const [name, entries] of byName) {
    ...
    // row would assert Argus considered an update it never could. Locations below are still
