...
// reads the clipboard with navigator.clipboard.readText(). Argus runs the renderer with
...
// (execCommand('copy') from a user gesture) both work. We route the read through Argus's
 ...
 * bridge); otherwise performs the paste through Argus's IPC clipboard read and
