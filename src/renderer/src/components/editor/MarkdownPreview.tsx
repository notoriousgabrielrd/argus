      ...
      a: ({ href, children, className, ...props }) => {
        ...
        const handleClick = async (event: React.MouseEvent<HTMLAnchorElement>): Promise<void> => {
          ...
          if (target.protocol === 'http:' || target.protocol === 'https:') {
            // Why: route through openHttpLink (not shell.openUrl) so a plain click honors "open links in Argus"; remote runtimes stay on the system browser.
            openHttpLink(
              target.toString(),
              resolveMarkdownPreviewHttpOpenOptions(
                event,
          ...
          if (!targetWorktree) {
            if (sourceRoutingWorktreeId && worktreeRoot) {
              // Why: floating markdown lives in a synthetic workspace with no repo worktree, though Argus can still open links relative to the source root.
