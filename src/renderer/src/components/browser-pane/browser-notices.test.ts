...
describe('browser notice formatting', () => {
  it('formats denied permissions with safe copy', () => {
    ...
    ).toBe('https://example.com asked for camera or microphone access, and Argus denied it.')
  })
  ...
  it('formats popup outcomes', () => {
    ...
    ).toBe('https://example.com opened a new page in Argus.')
    ...
    ).toBe('A site tried to open a popup Argus does not support here.')
  })
  ...
  it('formats certificate failures without local-server recovery advice', () => {
    ...
    expect(formatLoadFailureDescription(loadError(-202), meta)).toBe(
      "Argus doesn't trust the authority that issued the certificate for localhost:3443."
    )
    expect(formatLoadFailureDescription(loadError(-208), meta)).toBe(
      "Argus couldn't verify the certificate for localhost:3443."
    )
