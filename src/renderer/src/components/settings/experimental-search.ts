...
import { getNotchOverlaySearchEntry } from './notch-overlay-search-entry'
  ...
  (): SettingsSearchEntry[] => [
    ...
    },
    {
      ...
      description: translate(
        ...
        'Stops idle background agent terminals after the configured idle window and resumes supported sessions when opened again. Agent sleep preserves launch options for agents started by Argus; manually started agents may resume with current Argus defaults.'
      ),
      keywords: [
        ...translateSearchKeyword(
          'auto.components.settings.experimental.search.0d24759f14',
...
export function getExperimentalSearchEntry() {
  return {
    ...
    notchOverlay: findEntry(
      translate('auto.components.settings.experimental.search.notchOverlay.title', 'Notch Overlay')
    ),
