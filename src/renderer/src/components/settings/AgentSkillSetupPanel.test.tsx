...
const INSTALL_COMMAND = 'npx skills add https://github.com/stablyai/orca --skill argus-cli --global'
const UPDATE_COMMAND = 'npx skills update argus-cli --global'
...
function panelProps(
  ...
  return {
    title: 'CLI skill',
    description: 'Enables agents to use Argus workflows.',
...
describe('AgentSkillSetupPanel', () => {
  ...
  it('surfaces skill freshness under hideHeader for guided setup hubs', () => {
    ...
    expect(html).toContain('argus-linear')
  ...
  it('keeps update copy until the installed panel checks CLI prerequisites', () => {
    const html = renderPanel({
      ...
      preInstallNotice: 'Install the Argus CLI before running agent skill setup.'
    })
  ...
  it('refreshes first-install freshness only after the terminal re-check finishes', async () => {
    ...
    await renderInteractivePanel({ freshnessSkillName: 'argus-cli', onRecheck })
    await clickButton('Install')
  ...
  it('keeps the command failure authoritative over presence discovery', async () => {
    await renderInteractivePanel({ freshnessSkillName: 'argus-cli' })
    await clickButton('Install')

    await act(async () => {
      mocks.terminalProps.at(-1)?.onCommandFinished?.(1)
    })
    await rerenderInteractivePanel({ installed: true, freshnessSkillName: 'argus-cli' })
