...
describe('browser settings search copy', () => {
  ...
  it('keeps the pre-feature wording while inverting is off', () => {
    expect(getBrowserLinkRoutingDescription({ isMac: true })).toBe(
      "Open http(s) links in Argus's built-in browser — from the terminal, markdown, and the editor. ⇧⌘-click always uses your system browser."
    )
  ...
  // Why: "always" would be a lie once the chord can land in Argus, so the nested row
...
describe('browser link routing modifier copy', () => {
  ...
  it('names the destination the modifier actually reaches', () => {
    expect(getLinkRoutingModifierTitle(false)).toBe('Hold Shift to open in Argus')
  ...
  it('points the description at Argus only when links currently open externally', () => {
    expect(getLinkRoutingModifierDescription({ openLinksInApp: false, isMac: true })).toContain(
      "Argus's built-in browser"
    )
  ...
  // Why: the toggle is off by default, so present-tense "opens one in Argus" would
  // describe behavior the user does not have yet.
  it('phrases the Argus branch as enabled-state copy', () => {
    expect(getLinkRoutingModifierDescription({ openLinksInApp: false, isMac: true })).toContain(
      'When enabled'
    )
  })
...
describe('Link Routing description localization', () => {
  ...
  it('renders the Korean copy with the shortcut interpolated', async () => {
    ...
    expect(description).not.toContain("Argus's built-in browser")
    ...
    await i18n.changeLanguage('en')
    expect(getBrowserLinkRoutingDescription({ isMac: true })).toContain("Argus's built-in browser")
  })
