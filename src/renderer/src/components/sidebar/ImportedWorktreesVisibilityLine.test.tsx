...
describe('ImportedWorktreesVisibilityLine', () => {
  it('renders the compact repo-group line with expand and dismiss actions', () => {
    ...
    expect(markup).not.toContain('Argus found 4 worktrees')
    ...
    const groups = groupWorktreesByParentPath([
      {
        ...
        path: 'C:\\Repos\\Argus\\FeatureX'
      }
    ])

    expect(groups).toMatchObject([{ path: 'C:/Repos/Argus' }])
    expect(groups[0]?.path).not.toBe('C:\\Repos\\Argus')
  })
