  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Argus. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Argus. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Argus workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Argus. The project folder on disk will not be deleted.'
