    ...
    if (
      ...
      toast.error(
        translate(
          ...
          'That SSH host is already in Argus.'
        )
      )
      return 'validation-failed'
    }
  ...
  if (typeof ssh.resolveConfigHost !== 'function') {
    throw new Error(
      translate(
        ...
        'Restart Argus to finish applying the SSH config picker update.'
      )
    )
  }
...
/** Bulk-load ~/.ssh/config hosts into Argus’s host list (sidebar targets). */
  ...
  try {
    ...
    toast.success(
      translate(
        ...
        'Added {{value0}} host{{value1}} to Argus.',
