...
describe('getTerminalUrlOpenHint', () => {
  ...
  // Why: with links already opening in Argus, inverting still lands on the system
  // browser, so the hint must not promise Argus.
  it('keeps the system-browser wording when inverting but links open in Argus', () => {
    stubPlatform(true)
  ...
  it('names Argus when inverting and links open externally', () => {
    stubPlatform(true)
    expect(getTerminalUrlOpenHint({ openLinksInApp: false, modifierInverts: true })).toBe(
      'Click for actions, ⌘+click to open, or ⇧⌘+click to open in Argus'
    )
  })
  ...
  it('uses the Ctrl chord off macOS', () => {
    stubPlatform(false)
    expect(getTerminalUrlOpenHint({ openLinksInApp: false, modifierInverts: true })).toBe(
      'Click for actions, Ctrl+click to open, or Shift+Ctrl+click to open in Argus'
    )
  })
  ...
  it('omits the action-menu gesture when terminal link actions are disabled', () => {
    ...
    ).toBe('Ctrl+click to open, or Shift+Ctrl+click to open in Argus')
  })
})
...
describe('terminalUrlOpenHintOptionsFor', () => {
  ...
  // Why: openHttpLink refuses to route a remote-owned URL into Argus, so promising
  // "open in Argus" there would advertise a click that lands somewhere else.
  ...
  // can still reach Argus while some other pane's runtime is active.
  it('keeps inversion for a local pane while a remote runtime is active', () => {
    ...
    expect(getTerminalUrlOpenHint(options)).toContain('to open in Argus')
  })
})
