...
// (pty-startup-ingress.ts:26-29, 247-252), so the cooked-mode echo of Argus's
...
// Argus's One Dark terminal theme — the values that appear in the leaked text.
 ...
 * comes from Argus's code, not from the harness.
 */
...
describe('#12112 opencode startup OSC 10/11 replies on the local path', () => {
  ...
  it('renders no reply text when the tty echo is coalesced with program output', () => {
    ...
    // interactive shell, so bash's echo of Argus's reply shares a read with the shell's
