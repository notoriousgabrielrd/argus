...
export const STARTUP_CWD_FALLBACK_NOTICE =
  '\r\n[Argus opened this terminal at the workspace root because its saved start folder no longer exists.]\r\n'
...
export function connectPanePty(
  ...
  const registerEffectiveLaunchConfig = (
    ...
    if (!effectiveLaunchConfig) {
      if (metadata?.launchAgent) {
        // Why: daemon launch identity can outlive the process while Argus is
  ...
  const scheduleReattachIdleAgentCursorReset = (): void => {
    ...
    reattachIdleAgentCursorResetTimer = setTimeout(() => {
      ...
      // fire because the agent was already idle before Argus restarted.
      queueAgentIdleTerminalModeReset()
  ...
  const onTerminalKeyDown = (event: KeyboardEvent): void => {
    ...
    // Why: user shell frameworks (bash-preexec/iTerm2) can replace Argus's
  ...
  const onBell = (): void => {
    ...
    // standalone BEL some time after daemon snapshot reattach, even when Argus
  ...
  // callbacks to the correct Argus pane without resolving worktrees from cwd.
  // The key matches the `${tabId}:${leafId}` composite used for cacheTimerByKey
  // and agentStatusByPaneKey. Treat it as opaque outside Argus.
  const state = useAppStore.getState()
  ...
  const runDeferredConnect = (): void => {
    ...
    const schedulePendingStartupCommandDelivery = (): void => {
      ...
      if (!sshStartupShellReady) {
        if (sshShellReadyFallbackTimer === null) {
          // Why: some SSH shells cannot emit Argus's ready marker. Prefer the
    ...
    const reattachReplayResetSequence = (
      ...
      // Why: an alt-screen pane is a live TUI Argus just does not recognise as an agent, and the
