...
function copiedNote(packageFileName: string): string {
  return translate(
    ...
    'Command copied. Run it in a system terminal to install {{value0}}, then quit and reopen Argus.',
    {
...
export function LinuxPackageInstallRecoveryCard({
  ...
  const EXPLAINER = translate(
    ...
    'Copy the command and run it in a system terminal on the computer where Argus is installed. After it finishes, quit and reopen Argus to run the new version.'
  )
  ...
  const TRUST_NOTE = translate(
    ...
    'Argus checks the downloaded file against the release metadata at the moment it builds this command. The system package itself is not signature-checked, and Argus cannot vouch for the file after that point.'
  )
  ...
  const handleRetryAutomatic = useCallback(() => {
    ...
    // work mid-quit. Released by the effect above when a fresh recovery status says Argus stayed open.
