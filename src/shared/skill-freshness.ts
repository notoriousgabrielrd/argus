...
// or a plugin's bundled copy. Argus's global update never writes here, so the owner's
...
// Why: Argus's updater only ever passes --global, so a copy a project owns is not
// something Argus can act on — it has no remedy by design. Reporting it as global drift
...
// Why: a real read failure is a fact about the user's disk and stays actionable. Argus's
