/* eslint-disable max-lines -- Why: canonical transport-agnostic listener; parser, normalizer, per-CLI extractors, and endpoint writer share invariants that must not drift between Argus's main process and the relay. */
...
/** Per-listener-instance caches needing per-PTY teardown; Argus's main process and the relay each get their own, never shared. */
    ...
    if (!state.warnedEnvs.has(key) && state.warnedEnvs.size < MAX_WARNED_KEYS) {
      ...
      console.warn(
        `[agent-hooks] received ${env} hook on ${expectedEnv} server. ` +
          'Likely a stale terminal from another Argus install.'
      )
    }
  }
}
...
/** Rebuild a pane's working roster from a persisted snapshot; live activity confirms a seed, a complete task inventory may reap an unconfirmed one whose finish hook arrived while Argus was offline. */
export function seedClaudeSubagentRosterFromSnapshots(
  ...
  for (const snapshot of snapshots) {
    ...
    roster.set(snapshot.id, {
      ...
      // Why: the seed can be a phantom (child finished while Argus was down, SubagentStop lost); let a PRESENT background_tasks list omitting the id remove it, not gate the pane 'working' forever.
 ...
 *  the pane any more (its PTY died while Argus was down, so no finish hook could
...
function buildClaudeCachedLeadStatusPayload(
  ...
  // Why: default 'working' — a spawn proves activity even before the lead's first state-bearing event (e.g. Argus restarted mid-session).
  const lead = state.claudeLeadStateByPaneKey.get(paneKey)
