...
// the relay sends to Argus. Consumed by `src/relay/agent-hook-server.ts` (which
...
// between Argus's main process and the remote relay.
//
// Invariants both ends rely on:
// - The relay normalizes; Argus routes. The envelope's `payload` field has
//   already been through `normalizeHookPayload` (which calls
//   `parseAgentStatusPayload` → `normalizeAgentStatusObject`) on the relay
//   side. Argus's `ingestRemote` re-runs the canonical payload normalizer at
...
// - The wire `connectionId` is **always `null`**: a `connectionId` is Argus's
//   local handle on an `ssh2` connection, not a wire identity. Argus stamps the
...
//   POST body so Argus's warn-once protocol diagnostics still fire. The relay
...
// with the same value so Argus can attribute the event back to the right CLI.
...
/** Wire envelope for a single hook event flowing relay → Argus. */
export type AgentHookRelayEnvelope = {
  source: AgentHookSource
  paneKey: string
  /** Ephemeral Argus launch identity stamped into the PTY env for this process. */
  launchToken?: string
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know Argus's local connectionId. */
  connectionId: null
  /** Preserved from the relay-side normalized hook event so Argus can
  ...
  /** True when the relay is replaying its cache after Argus reconnects. */
  isReplay?: boolean
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets Argus's warn-once
   ...
   *  Argus's `ingestRemote` validates it again at the SSH trust boundary. */
...
/**
 * Re-attaches shed fields from Argus's cached payload for this pane, so a transport-level
...
/** JSON-RPC request method Argus issues after `--connect` reattach to ask the
 *  relay to replay its per-paneKey last-payload cache. Pull, not push: a relay
 *  that pushed on `setWrite` can emit before Argus has wired its `agent.hook`
...
/** JSON-RPC request method Argus issues at session-ready to ship the
...
export type AgentHookInstallManagedHooksParams = {
  /** SHA-256 fingerprint of the server key negotiated by Argus's SSH transport. */
  hostKeyFingerprint?: string
  /** Positively detected and enabled agents allowed to mutate remote config. */
  agents: readonly AgentHookTarget[]
}

/** Feature-flag env var. Read once at process start by Argus and the relay.
