...
/**
 * The community `skills` CLI's own `--agent` key for each agent Argus detects.
 *
 ...
 * rather than guessed. Argus ids and skills keys agree less often than they look
 * (`claude` is `claude-code`, `rovo` is `rovodev`, `aug` is `augment`), and some
 * near-matches are different products — Argus's `aider` CLI is not the CLI's
 * `aider-desk`, and Argus's `openclaude` is its `openclaw` in name only, so it
 * follows Argus's own rule that OpenClaude reads Claude-owned roots.
 */
export const SKILLS_CLI_AGENT_KEY_BY_TUI_AGENT = {
  claude: 'claude-code',
  'claude-agent-teams': 'claude-code',
  // Why: Argus states OpenClaude reads Claude-owned roots (native-chat-agent-profiles).
...
/** Map detected Argus agents onto `skills --agent` keys, plus the universal target. */
