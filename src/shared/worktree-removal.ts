...
// Only the detail after it is Argus's own wording, so verdict matchers anchor on the boundary
...
// making a different decision than one killing a terminal Argus just saw running. The marker
...
export function classifyWorktreeForceDeleteReason(
  ...
  if (isLockedWorktreeRemovalError(error)) {
    ...
    // unlocked explicitly rather than folded into Argus's dirty-file force path.
    return null
  }
