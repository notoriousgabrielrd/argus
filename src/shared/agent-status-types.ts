...
export type AgentStateHistoryEntry = {
   ...
   *  or Argus's guarded fallback). Always falsy for non-`done` states so retention logic can preserve it. */
  interrupted?: boolean
}
...
export type AgentStatusEntry = {
   ...
   *  (agent-reported or Argus's guarded fallback). Undefined otherwise. */
  interrupted?: boolean
   ...
   *  Used only for exact CLI resume; Argus terminal ids are not agent-session ids. */
