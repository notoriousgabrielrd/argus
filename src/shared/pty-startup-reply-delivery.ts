...
// whatever line discipline sits between Argus and the querying program can echo it
...
// A program that queries before clearing ECHO loses that race if Argus answers
...
export class PtyStartupReplyDelivery {
  ...
  answer(reply: string, onFailed?: () => void): boolean {
    ...
    if (!defersWrite(this.ownerBackend)) {
      // Why: ConPTY answers the query itself unless Argus beats it in this turn.
   ...
   * querying program left cooked mode, and the leak happens precisely because Argus
