...
export function verifyRemotePairingRuntimeStatus(
  ...
  if (status.deviceScope === 'mobile') {
    return {
      ...
      message: 'This link grants mobile-only access. Generate a link for another Argus client.'
    }
  }
  const versionFields = [
    ...
    return {
      ...
      message:
        compatibility.reason === 'client-too-old'
          ? 'Update this Argus client before adding the remote host.'
          : 'Update Argus on the remote host before adding it.'
    }
  }
