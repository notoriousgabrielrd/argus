...
describe('command token scanner', () => {
  ...
  it('preserves quoted command paths with spaces', () => {
    expect(getFirstCommandToken('"C:\\Program Files\\Argus\\codex.cmd" --resume')).toBe(
      'C:\\Program Files\\Argus\\codex.cmd'
    )
  })

  it('extracts path basenames without allocating path segment arrays', () => {
    expect(getCommandTokenPathBasename('C:\\Program Files\\Argus\\codex.cmd')).toBe('codex.cmd')
