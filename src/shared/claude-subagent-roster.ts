...
export type TrackedClaudeSubagent = {
   ...
   *  observed (Argus restart). A present complete task list omitting it
 ...
 *    never saw start (Argus/relay restart mid-run) → recreate it;
 ...
 *  `background_tasks` list; a parent that went idle before Argus restarted never
