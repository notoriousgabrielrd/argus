...
for (const [agent, config] of Object.entries(TUI_AGENT_CONFIG) as [
  ...
  for (const candidate of [
    ...
    if (normalized) {
      // Why: claude-agent-teams is an Argus wrapper whose child process is the
...
export function recognizeAgentProcessFromCommandLine(
  ...
  // Why: the generic Argus CLI is not an agent; only this subcommand launches its TUI mode.
