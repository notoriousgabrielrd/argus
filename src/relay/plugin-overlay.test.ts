...
describe('PluginOverlayManager', () => {
  ...
  it('mirrors a preexisting remote OpenCode config dir before adding Argus plugin', () => {
  ...
  it('installs Argus status extension into the remote default Pi agent dir', () => {
    const piAgentDir = join(homeDir, '.pi', 'agent')
...
describe('resolvePiSourceAgentDir', () => {
  ...
  it('keeps explicit PI_CODING_AGENT_DIR values when they are not Argus overlays', () => {
    const env = {
      HOME: mkdtempSync(join(tmpdir(), 'plugin-overlay-env-')),
