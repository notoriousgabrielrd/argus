...
async function fixture(options: { sha512?: string; url?: string } = {}) {
  ...
  const artifactName = 'argus-macos-arm64.zip'
...
describe('loadLocalBuildCandidate', () => {
  it('returns a sanitized, architecture-specific feed after hash validation', async () => {
    ...
    expect([...candidate.artifacts.keys()]).toEqual(['argus-macos-arm64.zip'])
    expect(candidate.manifestContent).toContain('argus-macos-arm64.zip')
  ...
  it('rejects mismatched hashes and traversal paths', async () => {
    ...
    const traversal = await fixture({ url: '../argus-macos-arm64.zip' })
    await expect(
  ...
  it('serves the same artifact descriptor that passed validation', async () => {
    ...
    try {
      await expect(
        fetch(`${feed.url}argus-macos-arm64.zip`).then((response) => response.text())
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      ...
      const resources = join(zipRoot, 'Argus.app', 'Contents', 'Resources')
      await mkdir(resources, { recursive: true })
      await writeFile(join(resources, 'orca-local-build.json'), JSON.stringify(compatibility()))
      const artifactName = 'argus-macos-arm64.zip'
      const artifactPath = join(directory, artifactName)
      await execFileAsync('/usr/bin/zip', ['-qry', artifactPath, 'Argus.app'], { cwd: zipRoot })
  ...
  it('requires exactly one ZIP for the running architecture', async () => {
    ...
    ).rejects.toThrow('exactly one x64 Argus ZIP')
  })
})
