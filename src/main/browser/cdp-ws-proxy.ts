...
export class CdpWsProxy {
  ...
  private handleHttpRequest(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (url === '/json/version' || url === '/json/version/') {
      ...
      // "Argus/CdpWsProxy" leaks that this is an embedded automation surface, which
  ...
  private handleClientMessage(client: WebSocket, raw: string): void {
    ...
    if (msg.method === 'Browser.getVersion') {
      // Why: returning "Argus/Electron" identifies this as an embedded automation
    ...
    // Use the same direct webContents reload path as Argus's own browser.reload.
  ...
  private async reloadWithLifecycle(
    ...
    if (unsupportedParam) {
      this.sendError(
        clientId,
        `Page.reload parameter "${unsupportedParam}" is not supported for Argus tab reloads`,
        client
      )
      return
    }
