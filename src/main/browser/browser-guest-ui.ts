...
export function setupGrabShortcutForwarding(args: {
  ...
  const handler = (event: Electron.Event, input: Electron.Input): void => {
    ...
    if (
      ...
      // Why: a focused guest swallows bare keys; during an active grab pick, plain C/S are Argus's copy/screenshot, not page typing.
...
export function setupGuestShortcutForwarding(args: {
  ...
  const forwardShortcutInput = (
    ...
    if (action?.type === 'zoom') {
      // Why: focused guest key events never reach the renderer-owned webview ref that applies Argus's page zoom.
