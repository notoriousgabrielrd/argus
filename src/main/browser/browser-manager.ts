...
export class BrowserManager {
        ...
        .executeJavaScript(
          `(function() {
            ...
            if (
              ...
              // Why: Argus remembers the last browser workspace/page even when
  ...
  attachGuestPolicies(
    ...
    // Why: only the primary embedded browser converts new-tab clicks to Argus tabs; OAuth child windows keep native link behavior.
    ...
    guest.setWindowOpenHandler(({ url, frameName }) => {
      ...
      if (browserTabId && canOpenAsChild) {
        ...
        return {
          ...
          // Why: default child windows lack an address bar; host in an Argus origin-bar window so the destination is verifiable.
  ...
  unregisterAll(): void {
    ...
    for (const downloadId of this.downloadsById.keys()) {
      this.cancelDownloadInternal(downloadId, 'Argus is shutting down.')
    }
  ...
  // Why: guests are isolated from Argus's preload bridge, so main owns the devtools escape hatch after a tab→guest lookup.
  ...
  private openLinkInOrcaTab(browserTabId: string, rawUrl: string): boolean {
    ...
    // Why: only the renderer owns Argus's worktree/tab model; main forwards a validated URL, never letting guest content mutate it.
