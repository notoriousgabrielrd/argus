...
const STAGING_ROOT = join(TEMP_ROOT, `argus-clipboard-files${UID_SUFFIX}`)
...
const LEGACY_EXPIRED = `argus-clipboard-file-${EXPIRED_TRANSFER}`
const LEGACY_FRESH = `argus-clipboard-file-${FRESH_TRANSFER}`
...
describe('legacy remote clipboard staging compatibility', () => {
  ...
  it('removes only exact expired legacy children encountered inside the bound', async () => {
    opendirMock.mockResolvedValue(
      openedDirectory([
        directoryEntry('argus-clipboard-file-lookalike'),
