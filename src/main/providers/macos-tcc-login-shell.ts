 ...
 * get their own TCC identity instead of collapsing into Argus's bundle id — signed
 * CLIs like `op` otherwise re-prompt every launch because tccd attributes the grant
 * to Argus and never persists it (#6996, #8985).
 *
...
export function wrapShellSpawnForMacosTccAttribution(
  ...
  // Why: Bash ignores --rcfile when argv[0] marks it as a login shell; Argus's
