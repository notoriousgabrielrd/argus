 ...
 * Argus descendant also reads `own`. It bounds the blast radius of a bad
 ...
 * usually belongs to an unrelated process whose chain never passes through Argus,
 ...
 * `own`. That is not remote during teardown, when Argus is itself the process
...
export function classifyWindowsTreeKillTarget(
  ...
  // `taskkill /T /F` on ourselves would take Argus and every pane down with it.
