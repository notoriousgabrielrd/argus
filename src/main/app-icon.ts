...
const MAC_DOCK_ICON_SCRIPT = [
  ...
  'if image is missing value then error "Argus app icon image could not be loaded"',
  "set ok to current application's NSWorkspace's sharedWorkspace()'s setIcon:image forFile:appPath options:0",
  'if ok is false then error "Argus app icon could not be persisted"'
]
...
const MAC_DOCK_ICON_CLEAR_SCRIPT = [
  ...
  'if ok is false then error "Argus app icon could not be cleared"'
]
