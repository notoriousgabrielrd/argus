  ...
  return {
    ...
    assertCurrent: () => {
      ...
      if (
        ...
        throw new Error(
          'The signed-in Argus account changed while the artifact request was running.'
        )
      }
    }
  }
...
function storedSessionAuthContext(
  ...
  if (!active.profile.cloud) {
    throw new Error('The active Argus profile is not linked to a cloud account.')
  }
  return authContext(
    active,
    {
  ...
  async update(request: ArtifactWriteRequest): Promise<ArtifactCloudOperation<ArtifactListItem>> {
    ...
    return this.withAuth(request, (token, apiUrl, auth) =>
      this.publisher.runForSource(request.sourceKey, auth, async () => {
        ...
        if (!record) {
          throw new Error('This file has not been shared from the active Argus profile.')
        }
