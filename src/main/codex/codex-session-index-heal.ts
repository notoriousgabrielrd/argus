...
// stamped `complete` on first app-server startup), so rollouts that Argus's
...
// Codex's DB-driven surfaces. Argus never writes Codex's sqlite schema itself.
...
function buildNativeHealInvocation(
  ...
  return {
    command: spawnCmd,
    args: spawnArgs,
    // Why: pin the real home explicitly — nested Argus launches can inherit a
