...
// Why: Pre/PostToolUse feed the live in-flight-tool readout; PermissionRequest exits with no decision so Codex still shows its approval UI while Argus flips the pane to waiting.
const CODEX_EVENTS = [
  'SessionStart',
  'UserPromptSubmit',
  'PreToolUse',
...
function writeCodexHooksJson(configPath: string, hooks: Record<string, HookDefinition[]>): void {
  // Why: Codex rejects unknown top-level hooks.json fields, so plugin bookkeeping like `_managed` must not survive Argus's rewrite.
...
const LEGACY_ORCA_PROFILE_BLOCK_START = '# BEGIN ARGUS AGENT STATUS HOOKS'
const LEGACY_ORCA_PROFILE_BLOCK_END = '# END ARGUS AGENT STATUS HOOKS'
...
function getTrustedSystemUserHookSignatures(
  ...
  try {
    ...
    // Why: a hand-broken system config.toml should only disable user-hook trust mirroring, not block Argus's managed runtime hooks.
...
function cleanupLegacySystemManagedHooks(): void {
  ...
  // Why: Codex hooks moved to Argus's managed CODEX_HOME; stale ~/.codex entries would keep external Codex sessions reporting into Argus.
  if (removedManagedHook) {
    // Why: this is the user's system hooks file, not Argus's runtime copy.
    // Remove only stale Argus hook entries and preserve other managers' metadata.
...
function cleanupLegacyCodexProfileHooks(): void {
  ...
  // Why: #2778 wrote Argus hooks into a Codex profile file; runtime CODEX_HOME supersedes it, so remove only Argus's marked block.
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    '#!/bin/sh',
    ...buildPosixHookPayloadCapture(),
    // Why: sourcing refreshes PORT/TOKEN/ENV/VERSION from the current Argus so a surviving PTY keeps reporting after a restart (see claude/hook-service.ts).
    'load_hook_endpoint() {',
    '  endpoint_path="$1"',
    '  case "$endpoint_path" in',
    '    *.cmd)',
    // Why: Windows passes endpoint.cmd into WSL via WSLENV; parse only Argus's known assignments since cmd.exe `set` lines aren't shell syntax.
...
function installManagedHooksIntoWslRuntime(
  ...
  try {
    ...
    if (grant.lane === 'fallback') {
      ...
      // here; only upsert Argus's entries instead of sweeping the whole source.
...
export class CodexHookService {
  ...
  install(runtimeHomePath: string = getOrcaManagedCodexHomePath()): AgentHookInstallStatus {
    ...
    try {
      ...
      if (grant.lane === 'rpc') {
        ...
        // all old runtime [hooks.state.*] blocks would keep Argus Codex trusted.
  ...
  async installRemote(
    ...
    remoteHome: string,
    options?: {
      /** Explicit CODEX_HOME dir (flat layout). WSL sessions read Argus's managed runtime home, not ~/.codex, so the default location leaves them hookless. */
      codexHomeDir?: string
    ...
    try {
      ...
      // Why: SSH remotes use POSIX `.sh` paths even when Argus runs on Windows; never derive remote script syntax from local OS.
  ...
  refreshRuntimeUserHooks(
    ...
    try {
      ...
      // Why: this path is used when Argus status hooks are disabled. The
