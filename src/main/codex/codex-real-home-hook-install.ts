 ...
 * - 'removed': hooks are opted out; Argus entries are swept from the real home.
 */
...
function assertHooksJsonGeneration(
  ...
  if (currentRaw !== expectedRaw || resolveHooksJsonWritePath(hooksJsonPath) !== hooksWritePath) {
    ...
    throw new Error('Codex hooks.json changed while Argus prepared its trust repair')
  }
}

/**
 * Ensures the real-home hook state matches the settings: installs and trusts
 * the Argus status hook when enabled, sweeps it when opted out. Idempotent and
...
function installRealHomeCodexHook(userDataPath: string): RealHomeCodexHookLane {
  ...
  // Why: sweep stale Argus entries out of events the managed lane no longer
  ...
  // Why: never leave an untrusted Argus entry in the user's real home — it
...
function reconcileManagedHookDefinition(
  ...
  if (!hasManagedDirectCommand && nestedLocations.length === 1) {
    ...
    if (definition.matcher === undefined && !hasDirectCommand) {
      ...
      // Why: users can append groups or handlers after Argus's first install.
...
function sweepRealHomeCodexHook(): RealHomeCodexHookLane {
  ...
  if (removedAny) {
    ...
    // groups must never make Argus delete a user's trust record at the same key.
    try {
      ...
      console.warn('[codex-real-home-hooks] failed to drop Argus trust entries:', error)
    }
  }
  return 'removed'
}

/** One-time pristine copy of the user's file, kept under Argus's userData. */
