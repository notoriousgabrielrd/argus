...
describe('promptForGpuFallbackRestart', () => {
  it('offers a restart without forcing it', async () => {
    ...
    expect(showMessageBoxMock).toHaveBeenCalledWith(parentWindow, {
      ...
      defaultId: 0,
      cancelId: 1,
      title: 'Restart Argus in Safe Graphics Mode?',
      message: "Argus's graphics process has crashed repeatedly.",
      detail:
        'Safe graphics mode disables hardware acceleration and WebGL for this Argus version. Terminals and 3D content may render more slowly. Keep Running leaves graphics settings unchanged.'
    })
  })
