...
describe('DaemonClient', () => {
  ...
  describe('connect', () => {
    ...
    it('captures one matching endpoint identity from both authenticated sockets', async () => {
      const identity = {
        ...
        entryPath: '/Applications/Argus.app/Contents/Resources/daemon-entry.js',
        appVersion: '1.2.3',
  ...
  describe('RPC', () => {
    ...
    it('adds recovery hints to node-pty daemon diagnostics', async () => {
      ...
      await expect(client.request('listSessions', undefined)).rejects.toThrow(
        "Daemon's node-pty install is gone (worktree deleted?). Restart Argus. node-pty: posix_spawn failed: ENOENT"
      )
    })
  })
