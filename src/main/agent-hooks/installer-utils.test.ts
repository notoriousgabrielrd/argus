...
describe('createManagedCommandMatcher', () => {
  ...
  it('matches commands containing the agent-hooks/<scriptFileName> path', () => {
    expect(
      match('/bin/sh "/Users/alice/Library/Application Support/Argus/agent-hooks/claude-hook.sh"')
    ).toBe(true)
  ...
  it('normalizes Windows backslashes so cmd-style paths still match', () => {
    expect(match('C:\\Users\\alice\\AppData\\Roaming\\Argus\\agent-hooks\\claude-hook.sh')).toBe(
      true
    )
  })
  ...
  it('matches the guarded launcher form so wrapped commands sweep correctly', () => {
    ...
    expect(
      match(
        'if [ -x "/Users/alice/Library/Application Support/Argus/agent-hooks/claude-hook.sh" ]; then /bin/sh "/Users/alice/Library/Application Support/Argus/agent-hooks/claude-hook.sh"; fi'
      )
    ).toBe(true)
  })
  ...
  it('removes managed direct bash/powershell/command fields', () => {
    const cleaned = removeManagedCommands(
      [
        {
          type: 'command',
          bash: '/bin/sh "/Users/alice/Argus/agent-hooks/copilot-hook.sh"',
          timeoutSec: 5
        },
        {
          type: 'command',
          powershell: "& 'C:\\Users\\alice\\Argus\\agent-hooks\\copilot-hook.sh'",
          timeoutSec: 5
        },
        {
          type: 'command',
...
describe('getSharedManagedScriptPath', () => {
  ...
  it('does not depend on Electron app.getPath, so two Argus instances resolve to the same path', () => {
...
describe('wrapPosixHookCommand', () => {
  ...
  it('preserves spaces in the script path (Library/Application Support case)', () => {
    ...
    const cmd = wrapPosixHookCommand('/Users/a/Library/Application Support/Argus/agent-hooks/x.sh')
    expect(cmd).toContain("'/Users/a/Library/Application Support/Argus/agent-hooks/x.sh'")
  })
