...
function buildAndApplyMenu(options: RegisterAppMenuOptions): void {
  ...
  const featureTourItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.exploreOrca', 'Explore Argus'),
    click: (_menuItem, window) => onOpenFeatureTour(window)
  }

  const setupGuideItem: Electron.MenuItemConstructorOptions = {
    label: translateMain('menu.gettingStarted', 'Getting Started with Argus'),
  ...
  // redundant "Argus" entry with roles that don't apply, so we omit it there
    ...
    submenu: [
      ...
      }),
      {
        ...
        click: () => {
          ...
          // control, so raw Electron paste cannot know which Argus surface owns it.
  ...
  const appearanceSubmenu: Electron.MenuItemConstructorOptions = {
    ...
    submenu: [
      ...
      {
        label: translateMain('menu.showMobileButton', 'Show Argus Mobile Button'),
        type: 'checkbox',
