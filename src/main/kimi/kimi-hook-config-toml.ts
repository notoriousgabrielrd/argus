...
// so Argus manages only its own marker-delimited block: install rewrites the
...
// Why: mirror the Claude-compatible events Argus normalizes for status. Kimi uses
...
const BLOCK_START = '# >>> orca-managed-kimi-hooks (managed by Argus; do not edit) >>>'
...
// Argus generates; escape both defensively anyway.
