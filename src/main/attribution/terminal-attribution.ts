...
const ORCA_GH_FOOTER = `Made with [Argus](${ORCA_PRODUCT_URL}) 🐋`
const SHELL_DOLLAR = '$'
...
export function applyTerminalAttributionEnv(
  ...
  // shim directory here keeps the attribution behavior scoped to Argus's live
...
if [[ -z "$real_git" ]]; then
  echo "Argus attribution wrapper could not locate git on PATH." >&2
  exit 127
fi
...
trailer="\${ORCA_GIT_COMMIT_TRAILER:-Co-authored-by: Argus <help@stably.ai>}"
...
if [[ -z "$real_gh" ]]; then
  echo "Argus attribution wrapper could not locate gh on PATH." >&2
  exit 127
fi
...
if [[ "\${1:-}" == "pr" && "\${2:-}" == "create" ]]; then
  footer="\${ORCA_GH_PR_FOOTER:-Made with [Argus](https://github.com/stablyai/orca) 🐋}"
...
if [[ "\${1:-}" == "issue" && "\${2:-}" == "create" ]]; then
  footer="\${ORCA_GH_ISSUE_FOOTER:-Made with [Argus](https://github.com/stablyai/orca) 🐋}"
  if has_passthrough_create_args "$@"; then
    PATH="$real_path" exec "$real_gh" "$@"
  fi
  if ! has_noninteractive_create_args "$@"; then
...
if defined ORCA_REAL_GIT (
  ...
  echo Argus attribution wrapper could not locate git on PATH. 1>&2
  exit /b 127
)
exit /b %ERRORLEVEL%
...
if defined ORCA_REAL_GH (
  ...
  echo Argus attribution wrapper could not locate gh on PATH. 1>&2
  exit /b 127
)
exit /b %ERRORLEVEL%
...
$trailer = if ($env:ORCA_GIT_COMMIT_TRAILER) { $env:ORCA_GIT_COMMIT_TRAILER } else { 'Co-authored-by: Argus <help@stably.ai>' }
...
if ($isPrCreate) {
  ...
  if ($createdUrl) {
    ...
    $footer = if ($env:ORCA_GH_PR_FOOTER) { $env:ORCA_GH_PR_FOOTER } else { 'Made with [Argus](https://github.com/stablyai/orca) 🐋' }
...
if ($isIssueCreate) {
  ...
  if ($createdUrl) {
    ...
    $footer = if ($env:ORCA_GH_ISSUE_FOOTER) { $env:ORCA_GH_ISSUE_FOOTER } else { 'Made with [Argus](https://github.com/stablyai/orca) 🐋' }
    if ($null -ne $body -and $body -notmatch [Regex]::Escape($footer)) {
      $tmpFile = [System.IO.Path]::GetTempFileName()
      try {
        $trimmed = $body.TrimEnd("${POWERSHELL_TICK}r", "${POWERSHELL_TICK}n")
