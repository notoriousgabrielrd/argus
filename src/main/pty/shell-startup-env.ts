...
// Why: Argus launches bash as a login shell (see local-pty-shell-ready.ts
...
// .bashrc. Scanning .bashrc would mirror values the live Argus bash never sees.
...
function shellStartupFiles(
  ...
  if (!shell) {
    // Why: Argus's POSIX default shell is /bin/zsh when $SHELL is unset.
  ...
  // Argus's zsh/bash/fish shell-ready startup files, so scanning those files
 ...
 *
 * Why: GUI-launched Argus does not inherit interactive shell exports, but the
