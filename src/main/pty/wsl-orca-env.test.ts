...
describe('addOrcaWslInteropEnv', () => {
  it('marks the Argus terminal handle for Windows to WSL env import', () => {
  ...
  it('path-translates a Windows hook endpoint but passes a guest-side one untouched', () => {
    const windowsEnv: Record<string, string> = {
      ORCA_AGENT_HOOK_ENDPOINT: 'C:\\Users\\jin\\AppData\\Roaming\\Argus\\agent-hooks\\endpoint.cmd'
    }
  ...
  it('never crosses a Windows OpenCode config dir into the guest', () => {
    ...
    // would adopt Argus's Windows overlay as its config root.
    const env: Record<string, string> = {
      OPENCODE_CONFIG_DIR: 'C:\\Users\\jin\\AppData\\Roaming\\Argus\\opencode-overlays\\abc',
      ORCA_OPENCODE_CONFIG_DIR: 'C:\\Users\\jin\\AppData\\Roaming\\Argus\\opencode-overlays\\abc'
    }
    addOrcaWslInteropEnv(env)
