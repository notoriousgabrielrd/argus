...
export class CodexAccountService {
   ...
   * instead of driving `codex login` here. Lets the `argus account add --agent codex`
  ...
  private async persistCapturedCodexAccount(
    ...
    if (!identity.email) {
      throw new Error('Codex login completed, but Argus could not resolve the account email.')
    }
  ...
  private createManagedHome(
    ...
    // Why: marker lets future cleanup prove the path belongs to Argus before deleting anything.
  ...
  private syncCanonicalConfigIntoManagedHome(
    ...
    // Why: Argus account switching is meant to swap Codex credentials and quota
    ...
    // Why: source-home Argus trust is foreign to each managed home's hooks.json.
  ...
  private assertOAuthAccountAddAllowed(canonicalConfig: CanonicalCodexConfig | null): void {
    ...
    throw new Error(
      `Argus cannot add a Codex OAuth account while ~/.codex/config.toml pins the custom provider ${JSON.stringify(modelProvider)}. Keep using the system-default account for this provider, or remove model_provider (or set it to "openai") before adding an OAuth account. Argus left your config unchanged.`
    )
  }
  ...
  private assertManagedHomePath(candidatePath: string, expectedAccountId?: string): string {
    ...
    if (wslInfo) {
      if (
        ...
        throw new Error('Managed WSL Codex home is outside Argus account storage.')
      }
      if (
      ...
      if (!existsSync(join(candidatePath, '.orca-managed-home'))) {
        throw new Error('Managed Codex home is missing Argus ownership marker.')
      }
      if (
        expectedAccountId !== undefined &&
