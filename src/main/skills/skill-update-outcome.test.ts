...
describe('skillUpdateFailedNames', () => {
  it('treats a convergent copy that is now current as landed', () => {
    expect(
      skillUpdateFailedNames(['argus-cli'], [placement('argus-cli', 'current')], noLocks)
    ).toEqual([])
  })

  it('reports a copy the run left outdated', () => {
    expect(
      skillUpdateFailedNames(['argus-cli'], [placement('argus-cli', 'outdated')], noLocks)
    ).toEqual(['argus-cli'])
  })
  ...
  it('reports a half-written bundle instead of reading it as success', () => {
    ...
    expect(
      skillUpdateFailedNames(['argus-cli'], [placement('argus-cli', 'unrecognized')], noLocks)
    ).toEqual(['argus-cli'])
  })

  it('reports an unreadable copy', () => {
    expect(
      skillUpdateFailedNames(['argus-cli'], [placement('argus-cli', 'inaccessible')], noLocks)
    ).toEqual(['argus-cli'])
  })

  it('reports a skill the run removed outright', () => {
    expect(skillUpdateFailedNames(['argus-cli'], [], noLocks)).toEqual(['argus-cli'])
  })
  ...
  it('accepts a revision newer than this build ships', () => {
    ...
    expect(
      skillUpdateFailedNames(['argus-cli'], [placement('argus-cli', 'newer-known')], noLocks)
    ).toEqual([])
  })
  ...
  it('ignores placements the update command never writes to', () => {
    expect(
      skillUpdateFailedNames(
        ['argus-cli'],
        [placement('argus-cli', 'current'), placement('argus-cli', 'outdated', 'plugin-cache')],
        noLocks
      )
    ).toEqual([])
  })
  ...
  it('fails the name when any convergent alias was left behind', () => {
    expect(
      skillUpdateFailedNames(
        ['argus-cli'],
        [placement('argus-cli', 'current'), placement('argus-cli', 'outdated', 'provider-alias')],
        noLocks
      )
    ).toEqual(['argus-cli'])
  })
  ...
  it('judges each requested name independently', () => {
    expect(
      skillUpdateFailedNames(
        ['argus-cli', 'orchestration'],
        [placement('argus-cli', 'current'), placement('orchestration', 'outdated')],
        noLocks
      )
  ...
  it('treats unrecognized content whose tree sha matches the lock as landed', () => {
    ...
    expect(
      skillUpdateFailedNames(
        ['argus-cli'],
        [placement('argus-cli', 'unrecognized', 'canonical-copy', 'ahead-of-bundle')],
        new Map([['argus-cli', 'ahead-of-bundle']])
      )
    ).toEqual([])
  })
  ...
  it('still reports unrecognized content whose bytes do not match the lock', () => {
    expect(
      skillUpdateFailedNames(
        ['argus-cli'],
        [placement('argus-cli', 'unrecognized', 'canonical-copy', 'half-written-bytes')],
        new Map([['argus-cli', 'ahead-of-bundle']])
      )
    ).toEqual(['argus-cli'])
  })
  ...
  it('never forgives an outdated copy, even at the lock hash', () => {
    ...
    expect(
      skillUpdateFailedNames(
        ['argus-cli'],
        [placement('argus-cli', 'outdated', 'canonical-copy', 'locked-revision')],
        new Map([['argus-cli', 'locked-revision']])
      )
    ).toEqual(['argus-cli'])
  })
  ...
  it('does not let a lock-matching canonical copy excuse a degraded alias', () => {
    expect(
      skillUpdateFailedNames(
        ...
        [
          placement('argus-cli', 'unrecognized', 'canonical-copy', 'ahead-of-bundle'),
          placement('argus-cli', 'inaccessible', 'provider-alias')
        ],
        new Map([['argus-cli', 'ahead-of-bundle']])
      )
    ).toEqual(['argus-cli'])
...
describe('skillUpdateFailedNames over a real inventory', () => {
  ...
  async function postCutFixture(): Promise<{ homeDir: string; installedTreeSha: string }> {
    ...
    const skillDir = join(homeDir, '.agents', 'skills', 'argus-cli')
    ...
    const current = await readFile(join(repoRoot, 'skills', 'argus-cli', 'SKILL.md'))
    await writeFile(
  ...
  it('accepts post-cut source content when the lock records exactly those bytes', async () => {
    ...
    const canonical = inventory.installations.filter(
      (entry) => entry.name === 'argus-cli' && entry.topology === 'canonical-copy'
    )
    expect(canonical).toHaveLength(1)
    expect(canonical[0].status).toBe('newer-known')
    expect(canonical[0].installedReleaseRevision).toBeNull()

    expect(skillUpdateFailedNames(['argus-cli'], inventory.installations, locks)).toEqual([])
  })
