...
const globallyUpdatableNames = new Set(['computer-use', 'argus-cli', 'orchestration'])
...
describe('skill freshness name-scoped update eligibility', () => {
  ...
  it.each([
    ...
    (status, topology) => {
      ...
      expect(
        eligible([
          placement('argus-cli'),
          placement('argus-cli', { id: `outlier-${status}-${topology}`, status, topology })
        ])
      ).toEqual(['argus-cli'])
    }
  )
  ...
  it.each(['unrecognized', 'inaccessible', 'newer-known'] as const)(
    ...
    (status) => {
      ...
      expect(
        eligible([
          placement('argus-cli', { id: 'blocked-canonical', status }),
          placement('argus-cli', {
            id: 'argus-cli-claude',
            rootId: 'home-claude',
            topology: 'provider-alias',
    ...
    expect(
      eligible([
        ...
        placement('argus-cli', {
          id: 'argus-cli-gemini',
          rootId: 'home-gemini',
          unresolvedPath: '/home/.gemini/skills/argus-cli',
          resolvedPath: '/home/.gemini/skills/argus-cli',
  ...
  it('does not offer an official canonical copy missing from the updater lock (#10791)', () => {
    expect(eligibleSkillUpdateNames([placement('argus-cli')], new Set())).toEqual([])
  })

  it('builds only an explicit, deterministic global command', () => {
    expect(buildTargetedSkillUpdateCommand(['orchestration', 'argus-cli', 'argus-cli'])).toBe(
      'npx skills update argus-cli orchestration --global'
    )
    expect(buildTargetedSkillUpdateCommand([])).toBeNull()
    expect(buildTargetedSkillUpdateCommand(['argus-cli;echo unsafe'])).toBeNull()
  })
})
