...
function userFacingUnsafeWindowsBatchArgs(label: string): string {
  return `${label} cannot be run as a Windows batch command with the prompt in argv. Remove {prompt} so Argus sends the prompt on stdin.`
}
...
// per-home lock keeps this run from racing Argus's own quota probes there.
