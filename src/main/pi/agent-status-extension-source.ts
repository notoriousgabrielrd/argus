...
// any Argus dep into the pi runtime.
...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  // so it cannot import from Argus's main bundle. fs/http coords come from
  // the same endpoint file the OpenCode plugin reads (process.env is frozen
  // at PTY spawn, so on Argus restart we have to re-read it from disk).
  return [
    ...
    '// Argus receiver from building an unbounded queue of obsolete snapshots.',
    ...
    '    timeout = setTimeout(() => {',
    '      controller?.abort()',
    "      reject(new Error('Argus hook delivery timed out'))",
    ...
    '    ])',
    '  } catch {',
    '    // Why: status reporting must never fail the pi run just because Argus',
    '    // is unavailable or the loopback request failed (e.g. Argus restart).',
