...
// Why: old Argus versions used per-kind overlay roots. Keep the names so
...
// shadowed one in Argus's per-launch agent picker.
...
export class PiTitlebarExtensionService {
  ...
  private getPtyOverlayDir(ptyId: string, kind: LegacyOverlayAgentKind): string {
    // Why: old Argus versions used PTY-scoped hashed overlays. Keep resolving
