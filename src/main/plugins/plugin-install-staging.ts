...
async function readInstallManifest(
  ...
  if (!satisfiesOrcaEngineRange(hostVersion, parsed.manifest.engines.orca)) {
    return {
      ok: false,
      error: `plugin requires Argus ${parsed.manifest.engines.orca} (this is ${hostVersion})`
    }
  }
...
export async function installStagedPluginTree(input: {
  ...
  if (blockedReason) {
    return { ok: false, error: `plugin is blocked by Argus's safety list: ${blockedReason}` }
  }
