...
const AMP_PLUGIN_MARKER = 'Managed by Argus. Do not edit; changes may be overwritten.'
  ...
  switch (state.kind) {
    ...
    case 'unmanaged':
      return {
        ...
        configPath: pluginPath,
        managedHooksPresent: false,
        detail: 'Amp Argus status plugin exists but is not Orca-managed'
      }
    case 'error':
      return {
        agent: 'amp',
...
function getAmpPluginSource(): string {
  return [
    ...
    '  // Why: Amp sessions can outlive an Argus restart; the endpoint file is',
    ...
    '    })',
    '  } catch {',
    '    // Why: Argus status reporting must never affect the Amp run.',
    '  } finally {',
    ...
    '  // payload closures when Argus is down and each POST waits for timeout.',
