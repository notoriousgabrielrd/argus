...
internal static class OrcaRemoteCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            ...
            {
                Console.Error.WriteLine("Argus SSH CLI bridge cannot find Node.js at \"{0}\"", nodePath);
                return 1;
            }
            if (!File.Exists(relayPath))
            {
                Console.Error.WriteLine("Argus SSH CLI bridge cannot find the relay at \"{0}\"", relayPath);
                return 1;
            }
        ...
        {
            Console.Error.WriteLine("Unable to start the Argus SSH CLI bridge: {0}", error.Message);
            return 1;
        }
    }
...
function createWindowsLauncherCompileCommand(
  ...
  return powerShellCommand(
    [
      ...
      "if (-not $compiler) { Write-Error 'Unable to find the .NET Framework C# compiler required for the Argus SSH CLI launcher.'; exit 1 }",
      `& $compiler ${compilerArgs}`,
      'if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }',
      `if (-not (Test-Path -LiteralPath ${powerShellLiteral(launcherPath)} -PathType Leaf)) { Write-Error 'The Argus SSH CLI launcher compiler produced no executable.'; exit 1 }`,
