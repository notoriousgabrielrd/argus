...
// switch (the cause of "Unsupported SSH Argus CLI command", #7716), the host
...
// Why: only Argus terminal-context vars may cross from the remote shell into
...
export async function runHostOrcaCliPassthrough(
  ...
  try {
    ...
    // reports "Argus is not running" against a healthy app.
  ...
  if (!entryExists(cliEntryPath)) {
    throw new HostCliUnavailableError(`Argus CLI entry not found at ${cliEntryPath}`)
  }
  ...
  return await new Promise<RemoteOrcaCliResult>((resolve, reject) => {
    ...
    const killTimer = setTimeout(() => {
      ...
      resolve({
        ...
        stderr: `${stderr.toString()}Argus CLI bridge timed out after ${killTimeoutMs}ms on the host.\n`,
        exitCode: 1
      })
    }, killTimeoutMs)
    killTimer.unref?.()
    ...
    child.on('error', (err) => {
      ...
      reject(
        new HostCliUnavailableError(`Failed to launch the Argus CLI on the host: ${err.message}`)
      )
    })
