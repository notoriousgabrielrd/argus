...
export function buildSshArgs(target: SshTarget, options?: SystemSshBuildArgsOptions): string[] {
  ...
  if (options?.gssapiOnly) {
    ...
    // credential prompt; failure belongs to Argus's existing ssh2 prompt path.
...
function shouldDisableOrcaControlMaster(
  ...
  // Why: unresolved ssh_config aliases could otherwise share one Argus socket
...
function hasUserConfiguredControlMaster(
  ...
  // created, but they do not create the setup-burst master Argus needs.
  return (
