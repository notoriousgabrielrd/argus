...
export type SystemTrayOptions = {
  ...
  /** Quit Argus for real (caller must set the quitting latch before quitting). */
  onQuit: () => void
}
...
function baseTooltip(): string {
  ...
  return devIndicator.label ? `Argus DEV (${devIndicator.label})` : 'Argus DEV'
}
      ...
      try {
        ...
        tray.setToolTip(
          devIndicator
            ...
            : translateMain('tray.activityWaiting', 'Argus - activity waiting')
        )
        return
      } catch (error) {
...
export function createSystemTray(opts: SystemTrayOptions): Tray | null {
  ...
  const menu = Menu.buildFromTemplate([
    ...(devIndicator
      ...
      : []),
    {
      label: translateMain('tray.openOrca', 'Open Argus'),
