...
describe('app icon selection', () => {
  ...
  it('persists a custom macOS dock icon to the app bundle for inactive Dock pins', async () => {
    ...
    persistMacDockIcon('watercolor', {
      appBundlePath: '/Applications/Argus.app',
      execFile,
      isDevApp: false,
      platform: 'darwin'
    })
    ...
    expect(execFile).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          ORCA_APP_BUNDLE_PATH: '/Applications/Argus.app',
  ...
  it('clears the AppKit icon and Finder metadata when switching macOS back to classic', async () => {
    ...
    expect(execFile).toHaveBeenCalledWith(
      '/usr/bin/xattr',
      ['-d', 'com.apple.FinderInfo', '/Applications/Argus.app'],
      expect.objectContaining({
        timeout: 10_000
      }),
      expect.any(Function)
    ...
    expect(execFile).toHaveBeenCalledWith(
      '/usr/bin/xattr',
      ['-d', 'com.apple.ResourceFork', '/Applications/Argus.app'],
      expect.objectContaining({
        timeout: 10_000
      }),
      expect.any(Function)
