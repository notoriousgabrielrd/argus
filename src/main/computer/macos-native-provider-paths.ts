...
export function resolveMacOSComputerUseAppPath(): string | null {
  ...
  const packaged = [join(process.resourcesPath ?? '', 'Argus Computer Use.app')]
  const dev = [
    join(process.cwd(), 'native/computer-use-macos/.build/release/Argus Computer Use.app'),
    resolve(__dirname, '../../native/computer-use-macos/.build/release/Argus Computer Use.app')
  ]
