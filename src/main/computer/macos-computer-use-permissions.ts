...
const DEFAULT_COMPUTER_USE_BUNDLE_ID = 'dev.argus.desktop.computer-use'
...
async function openComputerUsePermissionsAsync(
  ...
  if (!helperAppPath) {
    throw new RuntimeClientError('accessibility_error', 'Argus Computer Use.app was not found')
  }
...
function nextPermissionStep(
  ...
  return `Grant ${missing.id === 'accessibility' ? 'Accessibility' : 'Screen Recording'} to Argus Computer Use, then retry get-app-state.`
}
