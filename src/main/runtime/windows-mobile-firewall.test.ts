...
function environment(
  ...
  return {
    platform: 'win32',
    isPackaged: true,
    executablePath: "C:\\Users\\O'Brien\\Argus\\Argus.exe",
...
describe('windows mobile firewall', () => {
  it('inspects the exact executable, port, and selected interface profile', async () => {
    ...
    expect(script).toContain(
      "Get-NetFirewallApplicationFilter -PolicyStore ActiveStore -Program 'C:\\Users\\O''Brien\\Argus\\Argus.exe'"
    )
    ...
    expect(script).toContain("C:\\Users\\O''Brien\\Argus\\Argus.exe")
  ...
  it('repairs only Argus mobile pairing on private networks after elevation', async () => {
    ...
    expect(repairScript).toContain("-Name 'Argus.MobilePairing'")
    ...
    expect(repairScript).toContain("-Program 'C:\\Users\\O''Brien\\Argus\\Argus.exe'")
