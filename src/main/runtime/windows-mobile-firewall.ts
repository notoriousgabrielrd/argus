...
const FIREWALL_RULE_NAME = 'Argus.MobilePairing'
const FIREWALL_RULE_DISPLAY_NAME = 'Argus Mobile Pairing'
...
New-NetFirewallRule -Name ${quotePowerShell(FIREWALL_RULE_NAME)} -DisplayName ${quotePowerShell(FIREWALL_RULE_DISPLAY_NAME)} -Description 'Allows Argus Mobile to connect to this Argus desktop on private networks.' -Direction Inbound -Action Allow -Enabled True -Profile Private -Protocol TCP -LocalPort ${port} -Program ${quotePowerShell(executablePath)} -EdgeTraversalPolicy Block | Out-Null`
}
