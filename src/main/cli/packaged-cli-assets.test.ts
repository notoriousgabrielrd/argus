...
const linuxLauncherAsset = new URL('../../../resources/linux/bin/argus', import.meta.url)
  ...
  itRunsUnixShell(
    ...
    async () => {
      ...
      try {
        ...
        const launcherPath = join(launcherDir, 'argus')
        const electronPath = join(appDir, 'argus')
...
`,
        ...
        const commandPath = join(commandDir, 'argus')
  ...
  itRunsUnixShell('runs the AppImage CLI wrapper through APPDIR at runtime', async () => {
    ...
    try {
      ...
      const commandPath = join(root, 'argus')
      await mkdir(cliDir, { recursive: true })
      await writeFile(
        cliPath,
        `exports.main = (argv) => {
