const MANAGED_MARKER = '# Argus managed WSL CLI launcher'
const BRIDGE_MANAGED_MARKER = '# Argus managed WSL CLI PowerShell bridge'
...
try {
  ...
  if ($args.Count -lt 1) {
    throw 'Invalid Argus WSL CLI bridge invocation.'
  }
  ...
  if ($null -eq $Process) {
    throw 'Unable to start the Argus Windows CLI launcher.'
  }
  $Process.WaitForExit()
...
export function getBridgePathFromCommandPath(commandPath: string): string {
  ...
  return `${commandPath.replace(/\/\.local\/bin\/(?:orca|argus-ide)$/, '/.local/share/orca')}/orca-wsl-bridge.ps1`
}
...
export function buildRegistrationLockPrelude(commandPath: string): string {
  ...
  // Why: the per-distro queue only serializes one Argus process; flock covers
