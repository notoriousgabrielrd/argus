...
describe('WindowsUserPathRegistryReader', () => {
  it('reads Unicode REG_EXPAND_SZ values without expanding the stored text', async () => {
    const registry = registryModule('%LOCALAPPDATA%\\Argus;C:\\工具\\bin;C:\\Développement')
    ...
    await expect(reader.read()).resolves.toEqual({
      state: 'success',
      value: '%LOCALAPPDATA%\\Argus;C:\\工具\\bin;C:\\Développement',
      expandable: true
    })
  ...
  it('coalesces concurrent reads, caches status reads, and bypasses the cache when fresh', async () => {
    ...
    resolveRegistry(registryModule('C:\\Tools;C:\\Argus'))
    await expect(refreshed).resolves.toEqual({
      state: 'success',
      value: 'C:\\Tools;C:\\Argus',
      expandable: true
    })
  })
  ...
  it.each([
    ...
    const reader = new WindowsUserPathRegistryReader({
      platform: 'win32',
      registryLoader: async () => registryModule('%LOCALAPPDATA%\\Argus', type)
    })

    await expect(reader.read()).resolves.toEqual({
      state: 'success',
      value: '%LOCALAPPDATA%\\Argus',
      expandable
    })
  })
