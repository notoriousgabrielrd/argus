...
async function makeFixture(): Promise<{ userDataPath: string; resourcesPath: string }> {
  ...
  // The bundled argus launcher must exist for the shim to be written.
  mkdirSync(join(resourcesPath, 'bin'), { recursive: true })
  writeFileSync(join(resourcesPath, 'bin', 'argus'), '#!/usr/bin/env bash\n', 'utf8')
...
describe('ensureLinuxTerminalOrcaCliShimDir', () => {
  it('writes an executable bare-orca shim that execs the bundled argus launcher', async () => {
  ...
  it('memoizes per userDataPath and re-asserts the exec bit for a stale shim', async () => {
    ...
    expect(readFileSync(healedPath, 'utf8')).toContain('argus')
