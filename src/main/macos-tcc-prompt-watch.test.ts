...
describe('parseTccPromptEvent', () => {
  ...
  it('separates the accessing binary from the responsible app', () => {
    ...
    // The whole point of #9756: the dialog says Argus, but node did the access.
    expect(event?.responsibleIdentifier).toBe('dev.argus.desktop')
...
describe('isOrcaAttributedPrompt', () => {
  it('accepts the app and detached terminal helper across Argus build identities', () => {
    for (const id of [
      'dev.argus.desktop',
      'dev.argus.desktop.helper',
      'dev.argus.desktop.dev',
      'dev.argus.desktop.dev.helper',
      'dev.argus.desktop.local',
      'dev.argus.desktop.local.helper'
    ]) {
      expect(
        isOrcaAttributedPrompt({
  ...
  it('rejects unrelated services even when Argus is responsible', () => {
    expect(
      isOrcaAttributedPrompt({
        service: 'kTCCServiceMicrophone',
        accessingIdentifier: 'orca',
        responsibleIdentifier: 'dev.argus.desktop'
      })
    ).toBe(false)
  })
})
