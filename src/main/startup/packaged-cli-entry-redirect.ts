...
/**
 * Why: on Windows the bundled native launcher runs `Argus.exe <unpacked CLI entry>`
 * with ELECTRON_RUN_AS_NODE=1. When that env var is dropped (e.g. a wrapper or
 * shell that resets it), Argus boots as a GUI, loses the single-instance lock to
 ...
 * CLI's status.
 *
 * Security: the spawned program is always `execPath` (Argus.exe) and the script
...
export function maybeRedirectPackagedCliEntryLaunch(options: RedirectOptions = {}): RedirectResult {
  ...
  if (env[REDIRECT_ATTEMPT_ENV] === '1') {
    process.stderr.write('Unable to start the Argus CLI through Electron node mode.\n')
    return { redirected: true, status: 1 }
  }
  if (!exists(cliEntryPath)) {
    process.stderr.write(`Unable to locate the Argus CLI entrypoint at ${cliEntryPath}\n`)
    return { redirected: true, status: 1 }
  }
