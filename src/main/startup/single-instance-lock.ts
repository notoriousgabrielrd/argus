...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another Argus instance is already running for this userData profile; exiting this launch after requesting the existing window. If no Argus process is running, this may be an Electron/macOS single-instance lock failure.'
...
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] ORCA_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another Argus instance running for the same profile.'
...
/**
 * Why: Argus writes two canonical discovery files into `<userData>/`:
