...
async function readStdinWithinLimit(maxBytes: number): Promise<string> {
  ...
  for await (const chunk of process.stdin) {
    ...
    if (bytes > maxBytes) {
      throw new RuntimeClientError(
        'invalid_argument',
        'Artifact is too large for the Argus CLI transport. Use the browser upload page instead.'
      )
    }
    chunks.push(buffer)
  }
...
function requireOperation<T>(operation: ArtifactCloudOperation<T>): T {
  ...
  if (operation.status === 'reconnect-required') {
    throw new RuntimeClientError('authentication_required', 'Sign in to Argus and try again.')
  }
