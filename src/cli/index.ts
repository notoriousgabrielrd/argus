...
// Why: the SSH relay bridge executes this CLI on the Argus host while the
...
export async function main(
  ...
  try {
    ...
    // lookup so users do not get misleading "Argus is not running" failures for
...
async function runClaudeTeams(argv: string[], cwd: string): Promise<void> {
  try {
    // Why: everything after `argus claude-teams` belongs to Claude Code, not
    // Argus's own flag parser, so new Claude flags work without Argus changes.
