...
export class RuntimeClient {
    ...
    if (this.remotePairing) {
      ...
      return {
        ...
        result: {
          ...
          // that this client machine has a local Argus desktop process.
          app: {
            running: false,
            pid: null,
  ...
  private async checkOrchestrationContractCompatibility(timeoutMs: number): Promise<void> {
    ...
    if (!response.result.capabilities?.includes(ORCHESTRATION_CONTRACT_RUNTIME_CAPABILITY)) {
      throw new RuntimeClientError(
        ...
        'The connected Argus runtime does not support the current orchestration contract. No effects were applied.',
  ...
  async openOrca(timeoutMs = 15_000): Promise<RuntimeRpcSuccess<CliStatusResult>> {
    ...
    throw new RuntimeClientError(
      ...
      'Timed out waiting for an Argus desktop window. The runtime may still be running headlessly.'
    )
  }
}
...
function throwDesktopActivationBlocked(): never {
  throw new RuntimeClientError(
    ...
    'Argus is running headlessly, but it cannot open a desktop window safely because the persistent terminal provider is unavailable. Quit Argus normally and start the app again; do not use open -n.'
  )
}
