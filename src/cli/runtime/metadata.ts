  ...
  try {
    ...
    if (!metadata || !findTransport(metadata, 'unix', 'named-pipe') || !metadata.authToken) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        `Argus runtime metadata is incomplete at ${metadataPath}`
      )
    }
    return metadata
  } catch (error) {
    ...
    throw new RuntimeClientError(
      'runtime_unavailable',
      `Could not read Argus runtime metadata at ${metadataPath}. Start the Argus app first.`
    )
  }
}
...
export function getDefaultUserDataPath(
  ...
  // Why: in dev mode (and for parallel Argus instances), the Electron app writes
  // runtime metadata to a separate userData directory (e.g. `argus-dev`) to avoid
  ...
  if (platform === 'win32') {
    ...
    if (!appData) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        'APPDATA is not set, so the Argus runtime metadata path cannot be resolved.'
      )
    }
