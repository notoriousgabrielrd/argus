...
const ROOT_HELP_TEXT = `argus

Usage: argus <command> [options]

Startup:
  open                      Launch Argus and wait for the runtime to be reachable
  serve                     Start a headless Argus runtime server
  status                    Show app/runtime/graph readiness

Diagnostics:
  diagnostics memory        Collect a memory snapshot for Argus and managed terminals
...
Skills:
  skills list               List version-matched skill guides bundled with this Argus CLI
  skills get                Print a version-matched skill guide as Markdown
  skills install            Install bundled Argus skills globally via the community skills CLI
  skills update             Update already-installed Argus skills via the community skills CLI

Environments:
  environment add           Save a remote Argus runtime from a pairing code
  environment list          List saved remote Argus runtimes
  environment show          Show one saved remote Argus runtime
  environment rm            Remove a saved remote Argus runtime

Environment Recipes:
  vm recipe doctor          Validate a per-workspace environment recipe

Automations:
  automations list          List scheduled Argus automations
  automations show          Show one Argus automation
  automations create        Create a scheduled Argus automation
  automations edit          Edit an Argus automation
  automations remove        Remove an Argus automation and its run history
  automations run           Run an Argus automation now
  automations runs          List automation run history

Projects:
  project list              List durable projects known to Argus
...
Repos:
  repo list                 List repos registered in Argus
  repo add                  Add a project to Argus by filesystem path
...
Worktrees:
  ...
  worktree set              Update Argus metadata for a worktree
  worktree rm               Remove a worktree from Argus and git
  worktree ps               Show a compact orchestration summary across worktrees

Files:
  file open                 Open a workspace file in the Argus editor
  file diff                 Open a workspace file diff in the Argus editor
...
Common Commands:
  argus open [--json]
  argus serve [--port <port>] [--pairing-address <host>] [--mobile-pairing] [--no-pairing] [--project-root <path>] [--recipe-json] [--json]
  argus status [--json]
  argus diagnostics memory [--json]
  argus agent-context [--json]
  argus account add [--agent claude|codex] [--json]
  argus account list [--json]
  argus environment add --name <name> --pairing-code <code> [--json]
  argus environment list [--json]
  argus environment show --environment <selector> [--json]
  argus environment rm --environment <selector> [--json]
  argus worktree list [--repo <selector>] [--limit <n>] [--json]
  ...
  argus worktree show --worktree <selector> [--json]
  argus worktree current [--json]
  ...
  argus worktree rm --worktree <selector> [--force] [--run-hooks] [--json]
  argus worktree ps [--limit <n>] [--json]
  argus file open <path> [--worktree <selector>] [--json]
  argus file diff <path> [--staged] [--worktree <selector>] [--json]
  argus file open-changed [--mode edit|diff|both] [--worktree <selector>] [--json]
  argus terminal list [--worktree <selector>] [--limit <n>] [--include-visual-layouts] [--json]
  argus terminal show [--terminal <handle>] [--json]
  argus terminal read [--terminal <handle>] [--cursor <n>] [--limit <n>] [--json]
  ...
  argus terminal stop --worktree <selector> [--json]
  argus terminal create [--worktree <selector>] [--title <name>] [--command <text>] [--focus] [--json]
  argus terminal split [--terminal <handle>] [--direction horizontal|vertical] [--json]
  argus terminal switch [--terminal <handle>] [--json]
  argus terminal close [--terminal <handle>] [--tab] [--json]
  argus project list [--json]
  argus project setups [--project <id>] [--host <host-id>] [--json]
  argus project setup-existing-folder --project <id> --host <host-id> --path <path> [--kind git|folder] [--display-name <name>] [--json]
  argus project setup-clone --project <id> --host <host-id> --url <clone-url> --destination <path> [--display-name <name>] [--json]
  argus project setup-create --project <id> --host <host-id> [--setup-id <id>] [--path <path>] [--kind git|folder] [--display-name <name>] [--worktree-base-path <path>] [--git-username <name>] [--state ready|not-set-up|setting-up|error|unsupported] [--method imported-existing-folder|cloned|provisioned] [--json]
  argus project setup-update --setup <setup-id> [--display-name <name>] [--path <path>] [--worktree-base-path <path>] [--git-username <name>] [--kind git|folder] [--state ready|not-set-up|setting-up|error|unsupported] [--method legacy-repo|imported-existing-folder|cloned|provisioned] [--json]
  argus project setup-delete --setup <setup-id> [--json]
  argus repo list [--json]
  argus repo add --path <path> [--json]
  argus repo show --repo <selector> [--json]
  argus repo set-base-ref --repo <selector> --ref <ref> [--json]
  argus repo search-refs --repo <selector> --query <text> [--limit <n>] [--json]
...
Output Options:
  ...
  --pairing-code <code>      Connect to a remote Argus runtime using an orca://pair?... code
  --environment <selector>   Connect using a saved environment id or name
  --help                    Show this help message

Behavior:
  Most commands require a running Argus runtime. If Argus is not open yet, run \`argus open\` first.
...
Browser Workflow:
  1. Create or navigate:  argus tab create --url https://example.com
                          argus goto --url https://example.com
  2. Inspect the page:    argus snapshot
     (Returns an accessibility tree with element refs like e1, e2, e3)
     For concurrent workflows, prefer: argus tab list --json
     then reuse tabs[].browserPageId with --page <id> on later commands.
  3. Interact:            argus click --element e2
                          argus fill --element e5 --value "search query"
                          argus keypress --key Enter
  4. Re-inspect:          argus snapshot
...
Examples:
  ...
  $ argus diagnostics memory --json
  $ argus repo list
  $ argus worktree create --name agent-task --agent codex --prompt "hi"
  $ argus worktree create --repo name:argus --name cli-test-1 --issue 273
  $ argus worktree create --repo name:argus --name linear-task --linear-issue https://linear.app/stably/issue/STA-335/test-issue
  $ argus worktree create --name linear-task --linear-issue STA-335
  $ argus worktree show --worktree branch:Jinwoo-H/cli
  $ argus worktree current
  $ argus worktree set --worktree active --comment "waiting on review"
  $ argus worktree set --worktree active --linear-issue null
  $ argus worktree ps --limit 10
  $ argus file open-changed --mode diff
  $ argus file open src/App.tsx
  $ argus terminal create --worktree active --command "codex"
  $ argus terminal list --worktree path:/Users/me/orca/workspaces/orca/cli-test-1 --json
  $ argus terminal send --terminal term_123 --text "hi" --enter
  $ argus terminal wait --terminal term_123 --for exit --timeout-ms 60000 --json
  $ argus tab current --json
  $ argus tab show --page page_123 --json
  $ argus tab create --url https://example.com --profile work
  $ argus tab profile clone --page page_123 --profile work --json
  $ argus snapshot
  $ argus click --element e3
  $ argus fill --element e5 --value "hello"
  $ argus goto --url https://example.com/login
  $ argus keypress --key Enter
  $ argus eval --expression "document.title"
  $ argus tab list --json`
...
export function formatCommandHelp(spec: CommandSpec): string {
  const lines = [`argus ${spec.path.join(' ')}`, '', `Usage: ${spec.usage}`, '', spec.summary]
...
export function formatGroupHelp(specs: CommandSpec[], group: string): string {
  ...
  const lines = [`argus ${group}`, '', `Usage: argus ${group} <command> [options]`, '', 'Commands:']
  for (const spec of groupSpecs) {
    lines.push(`  ${spec.path.slice(1).join(' ').padEnd(18)} ${spec.summary}`)
  }
  lines.push('', `Run \`argus ${group} <command> --help\` for command-specific usage.`)
  return lines.join('\n')
}
...
export function formatFlagHelp(flag: string): string {
  const helpByFlag: Record<string, string> = {
    ...
    comment: '--comment <text>       Comment stored in Argus metadata',
    ...
    activate: '--activate             Reveal the new worktree in the Argus app',
    ...
    'display-name': '--display-name <name>  Override the Argus display name',
    ...
    page: '--page <id>            Stable browser page id from `argus tab list --json`',
  ...
  if (flag === 'current') {
    return '--current              Use the current Argus worktree linked Linear issue'
  }
  if (flag === 'comments') {
