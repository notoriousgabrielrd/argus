...
export const ORCHESTRATION_WORKER_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    notes: [
      ...
      '--on selects only the worker server; the Run and this command remain on the current Argus server.',
      ...
      'A worker whose terminal dies without reporting is replaced automatically in the same worktree, with the same agent and model, until the dispatch circuit breaker trips. Pass --no-auto-restart to leave the reclaimed task in the ready queue instead.',
  ...
  },
  {
    ...
    usage: 'argus orchestration worker-show --dispatch <dispatch_id> [--json]',
  ...
  },
  {
    ...
    usage:
      'argus orchestration worker-read --dispatch <dispatch_id> [--source <auto|transcript|terminal>] [--cursor <cursor>] [--limit <n>] [--json]',
    ...
    notes: [
      ...
      'A returned cursor is pinned to the exact source; start a fresh read if Argus reports source_changed.'
    ]
  },
  {
    ...
    usage:
      'argus orchestration worker-stop --dispatch <dispatch_id> [--retry-request <id>] [--json]',
  ...
  },
  {
    ...
    usage:
      'argus orchestration worker-abandon --dispatch <dispatch_id> [--retry-request <id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'dispatch', 'retry-request'],
  ...
  },
  {
    ...
    usage:
      'argus orchestration worker-release --dispatch <dispatch_id> [--retry-request <id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'dispatch', 'retry-request'],
    notes: [
  ...
  },
  {
    ...
    usage:
      'argus orchestration worker-retain --dispatch <dispatch_id> [--retry-request <id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'dispatch', 'retry-request'],
    notes: [
  ...
  },
  {
    ...
    usage:
      'argus orchestration worker-list [--run <run_id>] [--terminal-state <active|reclaimable|retained|release_pending|release_unknown|released>] [--json]',
