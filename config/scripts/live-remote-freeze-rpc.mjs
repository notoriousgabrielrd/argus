...
export function resolveOrcaCliInvocation({
  ...
  if (
    ...
    (commandName === 'argus-dev' || commandName === 'argus-dev.cmd')
  ) {
...
export function createOrcaRpc({
  ...
  async function runRestartProxy(notes) {
    ...
    try {
      ...
      notes.push(`argus open ms=${opened.elapsedMs.toFixed(0)}`)
    } catch (error) {
      notes.push(`argus open failed: ${String(error).slice(0, 200)}`)
    }
