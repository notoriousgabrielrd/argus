...
function applyCjkLatinTermSpacing(localeValue, locale) {
  ...
  if (locale === 'ko') {
    ...
    // boundary — re-glue "Argus 에"/"PR 을"/"에서는" but keep "Jira 이슈"/"Argus 로고"/"agent 에뮬레이터".
...
export function repairTranslatedValue({ key, enValue, localeValue, locale }) {
  ...
  if (enValue === 'Argus' || enValue.startsWith('Argus ')) {
    result = result
      .replaceAll('虎鲸', 'Argus')
      .replaceAll('逆戟鲸', 'Argus')
      .replaceAll('シャチ', 'Argus')
  }
...
export function repairCatalog(enCatalog, localeCatalog, locale) {
  ...
  if (localeCatalog.menu) {
    if (locale === 'zh') {
      if (localeCatalog.menu.exploreOrca !== '探索 Argus') {
        localeCatalog.menu.exploreOrca = '探索 Argus'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Argus 入门') {
        localeCatalog.menu.gettingStarted = 'Argus 入门'
        repaired += 1
      }
    }
    if (locale === 'ko') {
      if (localeCatalog.menu.exploreOrca !== 'Argus 둘러보기') {
        localeCatalog.menu.exploreOrca = 'Argus 둘러보기'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Argus 시작하기') {
        localeCatalog.menu.gettingStarted = 'Argus 시작하기'
        repaired += 1
      }
    }
  }
