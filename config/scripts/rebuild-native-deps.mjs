  ...
  if (isWindowsNativeLockError(err)) {
    console.error(
      '[rebuild] A Windows process appears to be using a native .node file. ' +
        'Close running Argus/Electron/dev processes for this worktree, then rerun `pnpm install` ' +
...
function getPatchedNodePtyRebuildReason() {
  ...
  // Why: Argus patches node-pty's native Unix spawn path; upstream prebuilds can
