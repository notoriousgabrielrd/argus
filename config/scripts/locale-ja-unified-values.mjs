...
export const JA_UNIFIED_VALUES = {
  ...
  'Show Argus Mobile Button': 'Argus Mobile ボタンを表示',
  Paste: '貼り付け',
  ...
  'Show the Argus Mobile button at the top of the left sidebar.':
    '左サイドバー上部に Argus Mobile ボタンを表示します。',
  'Enable Argus CLI': 'Argus CLI を有効化',
  'Registered the Argus CLI in PATH.': 'Argus CLI を PATH に登録しました。',
  ...
  'Gitea credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart Argus if environment variables changed.':
    'Gitea の認証情報は設定されていますが、認証できませんでした。トークン、API ベース URL、リポジトリの権限を確認し、環境変数を変更した場合は Argus を再起動してください。',
  'Azure DevOps credentials are configured but could not authenticate. Check the token, API base URL, and repository permissions, then restart Argus if environment variables changed.':
    'Azure DevOps の認証情報は設定されていますが、認証できませんでした。トークン、API ベース URL、リポジトリの権限を確認し、環境変数を変更した場合は Argus を再起動してください。',
  'Bitbucket credentials are configured but could not authenticate. Check the token and repository permissions, then restart Argus if environment variables changed.':
    'Bitbucket の認証情報は設定されていますが、認証できませんでした。トークンとリポジトリの権限を確認し、環境変数を変更した場合は Argus を再起動してください。',
