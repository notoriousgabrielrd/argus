...
const JA_AUDIT_OVERRIDES = {
  ...
  'Split Up': '上に分割',
  'Split Down': '下に分割',
  "Install the Browser Use skill so agents can operate Argus's browser.":
    'Browser Use スキルをインストールすると、 Agent が Argus のブラウザを操作できるようになります。',
  'bold ·': '太字 ·',
  ...
  '{{host}} is no longer a saved SSH host, so this workspace is no longer connected to a live host. It can only be removed from Argus — files and branches on the remote are left untouched.':
    '{{host}} は保存済みの SSH ホストではなくなったため、このワークスペースは稼働中のホストに接続されていません。Argus から削除することのみ可能です。リモート上のファイルとブランチはそのまま残ります。',
  'The app shell could not finish rendering. Retry to remount it, or relaunch Argus if the error persists.':
    'アプリシェルのレンダリングを完了できませんでした。再マウントを試すか、エラーが解決しない場合は Argus を再起動してください。',
  'Workspaces are unavailable on a mobile-scope pairing. Reconnect using the browser access link from Settings → Runtime Environments → Share this Argus server.':
    'モバイル向けペアリングではワークスペースを使用できません。[設定] → [リモート Argus サーバー] → [この Argus サーバーを共有する] から、ブラウザ用リンクで再接続してください。',
  'Change this later in Settings → Browser.': '後で [設定] → [ブラウザ] から変更できます。',
  'Argus Cloud sign-in is not configured': 'Argus Cloud へのサインインが設定されていません',
  ...
  'only when Argus cannot derive the API base URL from the git remote.':
    'は、Argus が git リモートから API のベース URL を取得できない場合のみ設定します。',
  'only when Argus cannot derive the API URL from the remote.':
    'は、Argus がリモートから API URL を取得できない場合のみ設定します。',
  Set: '設定:',
  '. Set': '。設定:',
  ', or set': '、または',
  and: 'と',
  ...
  ' vs {{value0}}': ' vs {{value0}}',
  'When you create a workspace, Argus refreshes the remote base and safely fast-forwards your matching local branch, such as':
    'ワークスペースを作成すると、Argus はリモートベースを更新し、一致するローカルブランチを安全に fast-forward します。対象は',
  '. This keeps commands like': '。これにより、',
  'from comparing against stale history. Argus skips the update if that branch has uncommitted changes or local-only commits.':
    'などのコマンドが古い履歴と比較されるのを防ぎます。対象のブランチにコミットしていない変更やローカルのみのコミットがある場合、Argus は更新をスキップします。',
  ...
  'Jump to...': '移動先…',
  'Start work': '作業を開始',
  'Remove {{value0}} contained {{value1}} from Argus':
    '含まれている {{value1}} {{value0}} 件を Argus から削除',
  'People in {{orgName}} who can collaborate on Argus.':
    '{{orgName}} で Argus を共同利用できるメンバーです。',
  'People in your organization who can collaborate on Argus.':
    '組織内で Argus を共同利用できるメンバーです。',
  ...
  'This QR code grants limited (mobile) access. To use the full web app, open the browser access link from Settings → Runtime Environments → Share this Argus server → New Link.':
    'この QR コードでは、モバイル向けの制限付きアクセスのみ利用できます。完全版の Web アプリを使用するには、[設定] → [リモート Argus サーバー] → [この Argus サーバーを共有する] → [新規リンク] から、ブラウザ用アクセスリンクを開いてください。',
  'Loading skills...': 'スキルを読み込み中…',
  'Start agents on your tasks without leaving Argus': 'Argus から離れずに、タスクから Agent を開始',
  'Workspace created from {{value0}}, but Argus could not fast-forward local {{value1}} because {{value2}}':
    'ワークスペースは {{value0}} から作成されましたが、{{value2}} のため Argus はローカル {{value1}} を fast-forward できませんでした',
  ...
  'Rename Orca-created branches from the initial agent task.':
    'Argus が作成したブランチの名前を、最初の Agent タスクに基づいて変更します。',
  ...
  'Command line Argus runs when a text recipe uses Custom command.':
    'テキストレシピで「カスタムコマンド」を使用したときに Argus が実行するコマンドライン。',
  ...
  'Repository not in Argus': 'このリポジトリは Argus に登録されていません',
  ...
  'Clears them from Argus only. Remote files, worktrees, and branches are left untouched.':
    'Argus からのみ削除します。リモートのファイル、ワークツリー、ブランチはそのまま残ります。',
  ...
  'Install the Argus CLI before running agent skill setup.':
    'Agent スキルのセットアップを実行する前に、Argus CLI をインストールしてください。',
  'Run grok in a terminal on the computer running Argus and wait for it to start. If prompted, complete sign-in, then retry usage. You do not need to send a chat message.':
    'Argus を実行しているコンピュータのターミナルで grok を実行し、起動するまで待ってください。サインインを求められた場合は完了してから、使用状況を再取得してください。チャットメッセージを送る必要はありません。',
  ...
  "Argus works with every CLI agent. Choose the one you'll reach for most. Switch any time.":
    'Argus はすべての CLI Agent で動作します。最もよく使うものを選択してください。いつでも切り替えられます。',
  ...
  'Install and manage experimental Argus plugins.':
    '実験的機能の Argus プラグインをインストール・管理します。',
  "Resident memory held by Argus plus the processes under each worktree's terminals.":
    'Argus が保持する常駐メモリと、各ワークツリーのターミナル配下で実行中のプロセス。',
  ...
  'Optional. Argus can use your normal Codex login; add accounts only if you want quick switching in Argus.':
    '任意。Argus は通常の Codex ログインを使用できます。Argus 上ですばやく切り替えたい場合にのみ、アカウントを追加してください。',
  'Optional. Argus can use your normal Claude login; add accounts only if you want quick switching without moving chat sessions.':
    '任意。Argus は通常の Claude ログインを使用できます。チャットセッションを移さずにすばやく切り替えたい場合にのみ、アカウントを追加してください。',
  'Optional. Argus works with your existing provider logins; add accounts only if you want Argus to help switch between them.':
    '任意。Argus は既存のプロバイダーのログインと連携します。Argus に切り替えを任せたい場合にのみ、アカウントを追加してください。',
