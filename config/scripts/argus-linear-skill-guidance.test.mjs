...
// Why: argus-linear and its legacy linear-tickets alias now ship hybrid discovery stubs, so
...
const canonicalGuidePath = join(projectDir, 'skill-guides', 'argus-linear.md')
const legacyGuidePath = join(projectDir, 'skill-guides', 'linear-tickets.md')
const canonicalStubPath = join(projectDir, 'skills', 'argus-linear', 'SKILL.md')
...
describe('argus-linear skill guidance', () => {
  it('keeps canonical and legacy Linear guide bodies from drifting', () => {
    ...
    expect(canonical).toContain('name: argus-linear')
...
describe('argus-linear install stubs', () => {
  const cases = [
    { name: 'argus-linear', stubPath: canonicalStubPath, guidePath: canonicalGuidePath },
