...
describe('orchestration skill guidance', () => {
  it('requires Argus runtime state before claiming a worker was orchestrated', () => {
    const skill = readSkill()
    const toolBoundary = getSection(skill, 'Tool Boundary')

    expect(toolBoundary).toContain('must create or bind a Run')
    expect(toolBoundary).toContain('create the Task with `argus orchestration task-create`')
    expect(toolBoundary).toContain('preferred `argus orchestration worker-start` composition')
    expect(toolBoundary).toContain('low-level `argus orchestration dispatch --inject` path')
    expect(toolBoundary).not.toContain('or `argus orchestration run`')
    ...
    expect(toolBoundary).toContain('do not create Argus task/dispatch provenance')
    ...
    expect(toolBoundary).toContain('argus orchestration task-list --json')
    expect(toolBoundary).toContain('argus orchestration dispatch-show --task <task_id> --json')
    expect(toolBoundary).toContain(
  ...
  it('keeps full handoffs out of dispatch lifecycle and off the active branch base', () => {
    ...
    expect(fullHandoffs).toContain(
      'Do not run `argus orchestration task-create`, `argus orchestration dispatch --inject`, or `argus orchestration check --wait` for full handoffs.'
    )
    expect(fullHandoffs).toContain(
    ...
    expect(skill).toContain(
      '`--no-parent` only controls Argus lineage; it does not choose the Git base.'
    )
    expect(skill).toContain(
      'never base it on the current feature branch unless the user explicitly asks'
    )
    expect(skill).toContain(
      'argus worktree create --name <task-name> --no-parent --agent codex --prompt'
    )
    expect(fullHandoffs).toContain(
      'Before creating a new worktree from an active feature branch, decide and state whether the desired Argus lineage is child or top-level'
    )
    expect(fullHandoffs).toContain(
  ...
  it('makes settled worker terminal release an explicit coordinator step', () => {
    ...
    expect(workerLoop).toContain(
      'argus orchestration worker-start --task <next_task_id> --terminal <handle> --json` so Argus ' +
    ...
    expect(workerLoop).toContain(
      'argus orchestration worker-retain --dispatch <dispatch_id> --json'
    )
    expect(workerLoop).toContain(
  ...
  it('keeps agent-first launch, handle recovery, and inbox injection distinct', () => {
    ...
    expect(agentFirstExample).not.toContain('argus terminal list')
...
describe('orchestration install stub', () => {
  ...
  it('does not tell agents to mutate orchestration state before loading the guide', () => {
    ...
    expect(preGuide).not.toContain('argus orchestration task-create')
    expect(preGuide).not.toContain('argus orchestration dispatch')
  })
