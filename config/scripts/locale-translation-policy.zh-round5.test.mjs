...
describe('locale-translation-policy zh round 5', () => {
  it('fixes brand spacing, hosted review, and Argus Mobile regressions', () => {
    expect(
      repairTranslatedValue({
        key: 'auto.components.stats.ShareUsageCard.0eb31e79ee',
        enValue: 'Argus IDE',
        localeValue: 'Argus集成开发环境',
        locale: 'zh'
      })
    ).toBe('Argus IDE')
    expect(
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Show Argus Mobile Button',
        localeValue: '显示 Argus 移动按钮',
        locale: 'zh'
      })
    ).toBe('显示 Argus Mobile 按钮')
    expect(
      repairTranslatedValue({
        key: 'auto.components.settings.GitPane.e02ea23a32',
        enValue: 'Argus Attribution',
        localeValue: 'Argus归属',
        locale: 'zh'
      })
    ).toBe('Argus 归因')
  ...
  it('keeps brand names English and generic workflow terms translated', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue:
          'Git could not safely delete branch "{{value0}}"{{value1}}, so Argus kept it to avoid losing local commits.',
        localeValue:
          'Git 无法安全删除分支“{{value0}}”{{value1}}，因此 Argus 保留它以避免丢失本地提交。',
        locale: 'zh'
      })
    ).toBe('Git 无法安全删除分支“{{value0}}”{{value1}}，因此 Argus 保留它以避免丢失本地提交。')
  })
