...
describe('skill bundle manifest generator', () => {
  ...
  it('rejects rewrites of released snapshots and allows floating-tail replacement', () => {
    ...
    const artifacts = {
      releasedSnapshotCounts: { 'argus-cli': 2 },
      snapshotRegistry: {
        schemaVersion: 1,
        skills: { 'argus-cli': [snapshot(1, 'aaa'), snapshot(2, 'bbb'), snapshot(3, 'ccc')] }
      }
    }

    expect(() =>
      assertReleasedHistoryPreserved(
        { schemaVersion: 1, skills: { 'argus-cli': [snapshot(1, 'aaa'), snapshot(2, 'bbb')] } },
        artifacts
      )
    ).not.toThrow()
    expect(() =>
      assertReleasedHistoryPreserved(
        {
          schemaVersion: 1,
          skills: { 'argus-cli': [snapshot(1, 'aaa'), snapshot(2, 'bbb'), snapshot(3, 'stale')] }
        },
        artifacts
      )
    ).not.toThrow()
    expect(() =>
      assertReleasedHistoryPreserved(
        {
          schemaVersion: 1,
          skills: { 'argus-cli': [snapshot(1, 'aaa'), snapshot(2, 'rewritten')] }
        },
        artifacts
      )
    ).toThrow('Released snapshot history changed for argus-cli at revision 2')
    expect(() =>
      assertReleasedHistoryPreserved(
        {
          schemaVersion: 1,
          skills: {
            'argus-cli': [snapshot(1, 'aaa'), { ...snapshot(2, 'bbb'), gitTreeSha: 'rewritten' }]
          }
        },
        artifacts
      )
      ...
      assertReleasedHistoryPreserved(
        {
          schemaVersion: 1,
          skills: {
            'argus-cli': [snapshot(1, 'aaa'), snapshot(2, 'bbb'), snapshot(3, 'stale')]
          }
        },
        { ...artifacts, releasedSnapshotCounts: { 'argus-cli': 1 } }
      )
    ).toThrow('Released snapshot history is incomplete for argus-cli')
  ...
  it('tolerates only redundant trailing release-mapping rows', () => {
    ...
    const rows = [
      { appVersion: '1.0.0', skills: { 'argus-cli': 1 } },
      { appVersion: '1.1.0', skills: { 'argus-cli': 2 } }
    ]
    const artifacts = {
      currentManifest: { skills: [{ name: 'argus-cli', releaseRevision: 2 }] },
    ...
    expect(
      isToleratedReleaseMappingPrefix(committedPrefix, {
        ...artifacts,
        currentManifest: { skills: [{ name: 'argus-cli', releaseRevision: 3 }] }
      })
    ).toBe(false)
    expect(
      isToleratedReleaseMappingPrefix(committedPrefix, {
        ...
        currentManifest: {
          skills: [
            { name: 'argus-cli', releaseRevision: 2 },
            { name: 'argus-linear', releaseRevision: 1 }
          ]
        }
      })
    ).toBe(false)
    ...
    expect(
      isToleratedReleaseMappingPrefix(
        serialized({
          schemaVersion: 1,
          releases: [{ appVersion: '0.9.0', skills: { 'argus-cli': 1 } }]
        }),
        artifacts
      )
    ).toBe(false)
  ...
  it('seeds released history from the committed ledger and drops the floating tail', () => {
    ...
    const committedRegistry = {
      ...
      skills: {
        ...
        'argus-cli': [snapshot(1, 'aaa'), snapshot(2, 'bbb'), snapshot(3, 'unreleased')],
        // no mapping row -> fall back to all-but-tail
        'argus-linear': [snapshot(1, 'ccc'), snapshot(2, 'tail')]
      }
    }
    const committedMapping = {
      schemaVersion: 1,
      releases: [{ appVersion: '1.0.0', skills: { 'argus-cli': 2 } }]
    }
    ...
    expect(seeded.registry.skills['argus-cli']).toEqual([snapshot(1, 'aaa'), snapshot(2, 'bbb')])
    expect(seeded.registry.skills['argus-linear']).toEqual([snapshot(1, 'ccc')])
    expect(seeded.releasedSnapshotCounts).toEqual({ 'argus-cli': 2, 'argus-linear': 1 })
  ...
  it('appends one release row, stripping the v-prefix and deduping identical tails', () => {
    const artifacts = {
      currentManifest: {
        skills: [
          { name: 'argus-cli', releaseRevision: 36 },
          { name: 'argus-linear', releaseRevision: 8 }
        ]
      },
      releaseMapping: {
        schemaVersion: 1,
        releases: [{ appVersion: '1.4.151', skills: { 'argus-cli': 35, 'argus-linear': 8 } }]
      }
    }
    ...
    expect(artifacts.releaseMapping.releases.at(-1)).toEqual({
      appVersion: '1.4.160',
      skills: { 'argus-cli': 36, 'argus-linear': 8 }
    })
  ...
  it('overwrites the trailing row when a failed cut is re-cut at the same version', () => {
    const artifacts = {
      currentManifest: { skills: [{ name: 'argus-cli', releaseRevision: 37 }] },
      releaseMapping: {
        schemaVersion: 1,
        releases: [
          { appVersion: '1.4.151', skills: { 'argus-cli': 35 } },
          // The failed cut already pushed this row to main at revision 36.
          { appVersion: '1.4.160', skills: { 'argus-cli': 36 } }
        ]
      }
    }
    ...
    expect(artifacts.releaseMapping.releases).toEqual([
      { appVersion: '1.4.151', skills: { 'argus-cli': 35 } },
      { appVersion: '1.4.160', skills: { 'argus-cli': 37 } }
    ])
  })
  ...
  it('computes the same Git tree identity as Git', async () => {
    const packageRoot = path.resolve('skills', 'argus-cli')
    const files = await collectPackageFiles(packageRoot)
    const expected = execFileSync('git', ['ls-tree', 'HEAD:skills', 'argus-cli'], {
      encoding: 'utf8'
    })
      .trim()
      .split(/\s+/)[2]
