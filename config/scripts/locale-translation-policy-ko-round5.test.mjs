  ...
  it('fixes Korean round 5 review, integration, and search keyword regressions', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Install the Argus skill so agents know to use the Argus CLI.',
        localeValue: '에이전트가 Argus CLI 사용 방법을 알 수 있도록 Argus 기술을 설치합니다.',
        locale: 'ko'
      })
    ).toBe('에이전트가 Argus CLI를 사용하도록 Argus 스킬을 설치하세요.')
    expect(
      repairTranslatedValue({
