...
if (process.platform !== 'win32') {
  ...
  // continue toward a Windows package whose declared argus.exe does not exist.
  throw new Error(
...
const sourcePath = join(repoRoot, 'native', 'windows-cli-launcher', 'ArgusCliLauncher.cs')
const outputPath = readArg('--output') ?? defaultOutputPath(repoRoot)
const compilerPath = findFrameworkCompiler(process.env)

if (!compilerPath) {
  throw new Error('Unable to find the .NET Framework C# compiler required for argus.exe.')
}
...
function defaultOutputPath(projectRoot) {
  return join(projectRoot, 'native', 'windows-cli-launcher', '.build', 'argus.exe')
}
