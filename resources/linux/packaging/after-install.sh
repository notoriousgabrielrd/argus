#!/bin/bash
# Why: register the bundled `argus` CLI on PATH at package-install time.
...
# symlink works. We discover the install dir instead of hardcoding /opt/Argus
# because electron-builder's directory name can vary by productName sanitization.
set -e

link="/usr/bin/argus"

for dir in /opt/Argus /opt/argus /opt/argus; do
  ...
  shim="$dir/resources/bin/argus"
  if [ -x "$shim" ]; then
    # Only manage our own symlink; never clobber an unrelated /usr/bin/argus.
