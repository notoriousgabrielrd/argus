...
description: >-
  ...
  Argus app UI, or desktop UI outside Argus's embedded browser.
---

# Argus Inter-Agent Orchestration

Orchestration is Argus's structured coordination layer for agent messages, task ownership, dispatch state, and worker completion tracking.

Use this skill when coordination state matters. For lightweight terminal prompts or basic worktree/terminal/built-in-browser control, use `argus-cli`.

## Tool Boundary

If a task says to use Argus orchestration, the coordinator must create or bind a Run, create the Task with `argus orchestration task-create`, then attach the worker with either the preferred `argus orchestration worker-start` composition or the low-level `argus orchestration dispatch --inject` path.

Do not substitute non-Orca subagent tools, generic agent-spawn APIs, or chat-only parallel worker features. Those may create useful workers, but they do not create Argus task/dispatch provenance, injected lifecycle preambles, `worker_done` authority, or decision gates.

Before claiming a worker was orchestrated, verify the task/dispatch exists:

```bash
argus orchestration task-list --json
argus orchestration dispatch-show --task <task_id> --json
```

If the work was accidentally run outside Argus orchestration, say so plainly. To repair provenance, rerun or revalidate the needed work through a fresh Argus terminal plus injected dispatch; do not retroactively describe the external worker as orchestrated.
...
- `argus status --json` should show a running runtime.
- `argus` must be on PATH.
- The orchestration experimental feature must be enabled in Settings > Experimental.
- `argus orchestration` commands are RPC calls to the running Argus runtime.

## Contract Migration

Argus adopts a live pre-update orchestration assignment into an ordinary Run. Adoption preserves the existing agent process, PTY/session, terminal handle, tab/leaf/pane, worktree or folder workspace, Task, and Dispatch; it never restarts or replaces the worker. The retired scheduler is not revived, and a newly created attempt uses the current grammar.
...
```bash
argus orchestration run-use --id <adopted_run_id> --takeover-legacy --json
argus orchestration check --run <adopted_run_id> --json
```
...
New orchestration messages and tasks belong to one explicitly bound Run. A Run is only a durable namespace and coordinator inbox; it never schedules or places workers. Lifecycle authority comes from the active Dispatch, and terminal handles remain routing metadata rather than durable identity. Send `worker_done` and `heartbeat` from the worker's own terminal; Argus routes them to that Dispatch's Run.
...
- Do not use `argus orchestration dispatch --inject` for full handoffs. It injects a coordinator preamble that tells the worker to send `worker_done`, heartbeat, and `ask` messages, then end its turn under the original terminal's dispatch lifecycle.
- Do not run `argus orchestration task-create`, `argus orchestration dispatch --inject`, or `argus orchestration check --wait` for full handoffs. Do not peek at terminal output after prompt delivery to monitor progress.
...
```bash
argus orchestration send --subject <text> [--to <run:id|dispatch:id|legacy_handle>] [--from <handle>] [--body <text>] [--type <type>] [--priority <level>] [--thread-id <id>] [--payload <json>] [--json]
argus orchestration check [--terminal <handle>] [--ack <delivery_id>] [--peek|--all] [--types <type,...>] [--format] [--wait] [--timeout-ms <n>] [--json]
argus orchestration reply --id <msg_id> --body <text> [--from <handle>] [--json]
argus orchestration ask (--question <text>|--resume <msg_id>) [--options <csv>] [--timeout-ms <n>] [--from <handle>] [--json]
argus orchestration inbox [--limit <n>] [--json]
```

Rules:

- Omit `--from` unless impersonating another terminal; Argus auto-resolves it from the current terminal.
...
- Use `dispatch:<id>` for coordinator guidance to one supervised worker. Argus routes that stable address locally or through the connected-server relay; do not substitute a remote terminal handle.
- Terminal handles remain appropriate for low-level pre-Dispatch messaging. Prefer `agentTerminalHandle` from the create response, fall back to `startupTerminal.handle` for older runtimes, then re-resolve with `argus terminal list --worktree ... --json` if missing or stale. Continue with the replacement handle only; never dual-send to old and new handles.
...
- `argus orchestration check --peek --format --json` returns locally formatted unread mail without consuming it; it never writes to terminal input or remotely wakes another terminal. Use `orchestration dispatch --inject` to deliver a tracked task, or `terminal send` when an existing agent needs a free-form prompt.
- While supervising workers manually, use `check --wait --types worker_done,escalation,question --timeout-ms <n>` instead of sleep/poll loops. Process the whole Delivery, reply to `question` messages with `argus orchestration reply --id <msg_id> --body <answer> --json`, then acknowledge and keep waiting.
...
- `heartbeat` is also Dispatch-scoped. Include both IDs and omit `--to` so Argus uses the owning Run; use `status` for broad progress updates.
...
```bash
argus orchestration run-create --objective <text> --json
argus orchestration task-create --spec <text> [--deps <json_array>] [--parent <task_id>] [--json]
argus orchestration task-list [--status <status>] [--ready] [--brief] [--json]
argus orchestration task-update --id <task_id> --status <status> [--result <json>] [--json]
argus orchestration dispatch --task <task_id> --to <handle> [--from <handle>] [--inject] [--json]
argus orchestration dispatch-show --task <task_id> [--json]
```
...
- If the target is a bare shell, omit `--inject`, dispatch for tracking if needed, then send the prompt manually with `argus terminal send --terminal <handle> --text <prompt> --enter --json`.
...
Use `worker-start` for the normal supervised path. It composes the existing worktree, terminal, readiness, and dispatch primitives while returning exact created/reused effects. Agents still choose placement and concurrency; Argus does not schedule workers or infer conflicts.

Create the Run and every independent Task first, then start all independent workers before waiting:

```bash
argus orchestration run-create --objective "<objective>" --json
argus orchestration task-create --spec "<worker A task>" --json
argus orchestration task-create --spec "<worker B task>" --json
argus orchestration worker-start --task <task_a> --worktree current --agent codex --json
argus orchestration worker-start --task <task_b> --worktree current --agent claude --json
```
...
```bash
argus orchestration worker-start --task <task_id> --worktree current --agent claude --model aws-bedrock-opus-5 --effort high --json
```

`--effort` requires `--model`, and neither option can combine with `--terminal`. A connected worker server must advertise launch-preference support before Argus forwards either option.

For a new worktree, setup runs by default and agent-first creation reuses the returned startup agent terminal:

```bash
argus orchestration worker-start --task <task_id> --worktree new-child --name <name> --agent codex --setup run --json
# Independent/top-level:
argus orchestration worker-start --task <task_id> --worktree new-top-level --name <name> --agent codex --setup run --json
```
...
To run the worker on another connected Argus server, add `--on <saved-environment>`. The Run and Tasks remain authoritative on the current server; later commands route by Dispatch ID, so never repeat `--on`:

```bash
# Mac Run home -> Windows worker (the reverse is identical from a Windows Run home)
argus orchestration worker-start --task <task_id> --on windows --worktree new-top-level --repo <exact_remote_repo_selector> --name <name> --agent codex --setup run --json
argus orchestration worker-show --dispatch <dispatch_id> --json
argus orchestration worker-read --dispatch <dispatch_id> --limit 50 --json
argus orchestration send --to dispatch:<dispatch_id> --subject "Follow-up" --body "<attempt-specific guidance>" --json
```
...
`orchestration check` receives it even when the Dispatch is on another connected Argus server.

`worker-read` defaults to `--source auto`: Argus returns the exact hook-reported Codex, Claude, OpenClaude, or Grok transcript when it can prove the worker session, otherwise it returns bounded terminal output with `source: "terminal"` and a typed `fallbackReason`. Continue with the returned top-level `cursor`; it stays pinned to that exact source. If Argus reports `source_changed`, start a fresh read without the old cursor. Never supply or guess a provider session ID or transcript path.

Wait until every expected Dispatch settles, not for a fixed number of batches:

```bash
argus orchestration check --wait --types worker_done,escalation,question --timeout-ms 900000 --json
# Process every message. For each accepted worker_done that is not immediately reused:
argus orchestration worker-release --dispatch <dispatch_id> --json
# Acknowledge only after every message and required release decision is handled:
argus orchestration check --ack <delivery_id> --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```

After processing each accepted `worker_done`, choose the terminal's next owner before you acknowledge the Delivery or wait again. If the same exact agent has an immediate follow-up Task, read the `worker.agent_terminal_handle` field of `worker-show --dispatch <dispatch_id> --json`, then run `argus orchestration worker-start --task <next_task_id> --terminal <handle> --json` so Argus transfers cleanup ownership to the new Dispatch. Otherwise run `argus orchestration worker-release --dispatch <dispatch_id> --json`.

Run `worker-release` after both succeeded and failed `worker_done` reports unless the user explicitly asked to keep that worker live. Release is post-completion cleanup, not cancellation: Argus first preserves inspectable output, then closes only the exact agent terminal owned by that settled Dispatch. Reused or pre-existing terminals, setup terminals, coordinators, active workers, user-taken-over terminals, and identities Argus cannot prove are retained. If the user explicitly asks to keep the live terminal for debugging, record that exception with `argus orchestration worker-retain --dispatch <dispatch_id> --json` instead of silently skipping cleanup. When the user is finished, the same Dispatch can be passed to `worker-release`, which clears the requested retention and releases the terminal.
...
Workers report exactly once using the IDs and capability injected by Argus; they do not supply Run/server/terminal identity:

```bash
argus orchestration send --type worker_done --subject "<status>" --body "<what changed, findings, and what remains>" --task-id <task_id> --dispatch-id <dispatch_id> --outcome succeeded --files-modified "path/a,path/b" --json
...
```bash
argus orchestration ask --question "<question>" --options "yes,no" --timeout-ms 600000 --json
argus orchestration ask --resume <message_id> --timeout-ms 600000 --json
# Coordinator:
argus orchestration reply --id <message_id> --body "<answer>" --json
```
...
```bash
argus orchestration gate-create --task <task_id> --question <text> [--options <json_array>] [--json]
argus orchestration gate-resolve --id <gate_id> --resolution <text> [--json]
argus orchestration gate-list [--task <task_id>] [--status <status>] [--json]
```
...
Recovery only: `argus orchestration reset --tasks|--messages|--all --json` clears the selected local orchestration database state. Do not run it during active coordination unless explicitly abandoning that state.
...
Do not run `argus orchestration task-create`, `argus orchestration dispatch --inject`, or `argus orchestration check --wait` for full handoffs. `task-create` is also forbidden because it records coordinator-owned tracking state; if a task row is needed, the user asked for supervised orchestration. Do not create a `taskId`/`dispatchId`, inject a lifecycle preamble, wait for completion, or read the worker terminal after prompt delivery except to avoid losing the initial prompt.

New top-level worktree handoff:

```bash
argus worktree create --name <task-name> --no-parent --agent codex --prompt "<task brief>" --setup run --json
```

Before creating a new worktree from an active feature branch, decide and state whether the desired Argus lineage is child or top-level. Use child worktree lineage only when the new work is conceptually stacked under or dependent on the active worktree. For independent repo-wide fixes, standalone feature work, or unrelated follow-up tasks, create a top-level worktree with `--no-parent`.

Existing terminal handoff:

```bash
argus terminal send --terminal <handle> --text "<task brief>" --enter --json
```

Custom Codex model/effort handoff:

`argus worktree create --agent codex --prompt ...` launches the known Codex agent but does not accept Codex-specific `--model` or `-c model_reasoning_effort=...` arguments. When the user asks for a specific Codex model or effort, create the independent worktree first, launch Codex with the requested command in that worktree, wait only for TUI readiness if prompt delivery would otherwise race startup, send the prompt, and stop.
...
Use the exact full `<repo-id>::<path>` worktree id returned by `argus worktree create --json`; a bare repo id cannot target the new worktree.

```bash
argus worktree create --name <task-name> --no-parent --setup run --json
argus terminal create --worktree id:<newFullWorktreeId> --title <task-name> --command 'codex --model gpt-5.5 -c model_reasoning_effort="xhigh"' --json
argus terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
argus terminal send --terminal <handle> --text "<task brief>" --enter --json
```

Wait only for `tui-idle` when needed to avoid losing the prompt. Do not monitor task completion.

`--no-parent` only controls Argus lineage; it does not choose the Git base. If the work should start from the repo default base, omit `--base-branch` so Argus uses that default, or explicitly pass the repo default base (`origin/main`, `origin/master`, or the `argus repo show --repo <selector> --json` value); never base it on the current feature branch unless the user explicitly asks for stacked work or "branch from current". Put current-branch context in the prompt instead.
...
```bash
argus terminal create --worktree active --title <task-name> --command "codex" --json
argus terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
argus orchestration dispatch --task <task_id> --to <handle> --inject --json
```
...
When a new worktree is allowed, use child lineage for isolated work that is stacked under or dependent on the active worktree, and use `--no-parent` when it is not stacked. Decide the Git base separately: `--no-parent` makes the worktree top-level in Argus, while omitted `--base-branch` uses the repo default base.
...
```bash
argus worktree create --name <task-name> --agent codex --setup run --json
...
Use `argus worktree create --prompt ...` or `argus terminal send ...` for full handoffs or untracked/lightweight prompts. Those paths do not attach `taskId`/`dispatchId`; the worker should not send lifecycle messages unless the prompt supplies a live orchestration preamble.
...
```bash
argus terminal list [--worktree <selector>] [--include-visual-layouts] [--json]
argus terminal create [--worktree <selector>] [--title <text>] [--command <cmd>] [--json]
argus terminal split --terminal <handle> [--direction horizontal|vertical] [--command <cmd>] [--json]
argus terminal wait --terminal <handle> --for tui-idle --timeout-ms <n> --json
argus terminal read --terminal <handle> --json
argus terminal send --terminal <handle> --text <text> --enter --json
```

If an older CLI rejects `worktree create --agent`, create the worktree normally, then run `argus terminal create --worktree <selector> --command "codex" --json` or `--command "claude"`.
...
- Workers with a valid live preamble must send `worker_done` exactly once from their own terminal with an explicit `--outcome succeeded` or `--outcome failed`:
  `argus orchestration send --type worker_done --subject "<short status>" --body "<3-sentence summary: what you did, what you found, what's left>" --task-id <task_id> --dispatch-id <dispatch_id> --outcome succeeded --files-modified "path/a" --report-path "<optional>" --json`
- A failed outcome is still a terminal report, but Argus records both the Dispatch and Task as failed. Never encode failure only in the subject/body.
...
- For long tasks, send heartbeat/status only when the preamble asks for it, including both IDs:
  `argus orchestration send --type heartbeat --subject "alive" --payload '{"taskId":"<task_id>","dispatchId":"<dispatch_id>","phase":"implementing"}' --json`
...
```bash
argus terminal create --worktree active --title login-css-worker --command "claude" --json
argus terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
argus orchestration task-create --spec "Fix the login button CSS" --json
argus orchestration dispatch --task <task_id> --to <handle> --inject --json
argus orchestration check --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```
