...
# Argus Emulator (serve-sim powered)

Drive an Apple Simulator (iOS / iPad / Watch) **from within Argus** using `ORCA emulator ...` commands (or `ORCA emulator exec` for raw power). This wraps the excellent [serve-sim](https://github.com/EvanBacon/serve-sim) open-source tool so agents get a consistent Orca-native CLI surface, automatic helper management, and seamless integration with Argus's live emulator pane (the visual "preview" surface).

The underlying serve-sim helper captures the real simulator framebuffer (via private SimulatorKit / IOSurface for low-latency 60fps H.264 or MJPEG) and exposes a WebSocket control channel. Argus's bridge owns the helper processes and per-worktree "active emulator" state so unqualified commands "just work" on whatever device/pane is current for the worktree.
...
- The user/agent wants to **tap, swipe, drag, pinch, or press hardware buttons** on a running iOS simulator while seeing the live result in Argus.
...
- You are inside an Argus worktree/terminal and want the emulator to be **workspace-scoped** (like browser tabs) with explicit targeting when needed.
- The agent should use Argus's preview pane instead of external Simulator.app or raw serve-sim URLs.

**When NOT to use**
- Android emulators → use the `argus-emulator-android` skill (same `ORCA emulator` namespace, cross-platform via adb/emulator).
...
## Prerequisites (enforced / surfaced by Argus)

- macOS host (with Xcode Command Line Tools: `xcrun --version`).
- A booted simulator (`xcrun simctl list devices booted` or let Argus/attach help boot one).
- Node available (for the serve-sim bits; Argus bundles the CLI surface).
- macOS 14+ recommended for full camera injection features.

Argus will give clear errors if these are missing (e.g. "emulator commands require macOS + Xcode tools").
...
│ Argus EmulatorBridge│ ───────────────► │ serve-sim-bin   │ ────────────► │ iOS Simulator│
│ (main process)     │ (or exec serve-sim) (per-device)   │               └──────────────┘
└────────────────────┘                  └─────────────────┘
          ▲
          │ (state + lifecycle)
┌────────────────────┐
│ argus CLI (agents)  │  e.g. ORCA emulator tap 0.5 0.7
│ argus-emulator skill│
└────────────────────┘
```
...
Agents use the Argus executable chosen above (on PATH in Argus terminals) and never have to manage PIDs, state files in /tmp, or raw WS URLs themselves.

**For `pnpm dev` testing:** run `pnpm build:cli` first (rebuilds the CLI + ensures the `argus-dev` shim points at *this* worktree). Then inside the dev app use `argus-dev emulator ...` (or the direct `./config/scripts/argus-dev.mjs emulator ...` from the repo root). The orchestration preambles and dev launchers automatically select the dev command name so the CLI reaches your in-memory EmulatorBridge / runtime. Plain `orca` reaches a packaged install instead.
...
| Goal                        | Command                                      | Notes |
|-----------------------------|----------------------------------------------|-------|
| List available / running   | `ORCA emulator list [--worktree <sel>]`     | Shows Argus-managed + raw serve-sim streams. Use output for explicit --device/--emulator. |
...
| Stop                       | `ORCA emulator kill [--device <id>]`        | Or let pane close / Argus quit clean up. |

Most support `--worktree <selector>` and explicit `--device <udid|name>` or `--emulator <id>` (from list) for targeting.

## Critical gotchas (teach agents)

- **Prefer `tap` over `gesture` for single taps** (same as raw serve-sim). Separate gesture begin/end can be interpreted as long-press due to WS overhead. The Argus wrapper uses the reliable quick sequence.
...
- Stale helpers / state are cleaned by Argus on quit, but agents should `kill` when done.
- Private APIs under the hood (SimulatorKit etc.) — version sensitive (Xcode updates can affect).

## Targeting devices & worktrees

- Default: current worktree's active emulator (resolved from shell cwd or Argus context).
...
- Opening the emulator pane in Argus (or `attach`) makes that stream the "active" one for the worktree → CLI commands target it automatically.
...
Or let Argus quit / close the pane.

Orphans are cleaned by Argus (like agent-browser sessions).
...
Confirm `ORCA status --json` and `ORCA emulator list --json`, then drive the emulator while the live view is visible in Argus.

See also: argus-cli skill (terminals, worktrees, built-in browser), computer-use for desktop outside the simulator.
