...
Windows, Linux, and macOS — unlike the iOS backend (`argus-emulator`), which is
...
- iOS simulators → use the `argus-emulator` skill (macOS only).
...
See also: `argus-emulator` (iOS, macOS-only), `argus-cli` (terminals, worktrees,
