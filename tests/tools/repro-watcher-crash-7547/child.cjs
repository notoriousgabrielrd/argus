...
// watches our exit code). Each scenario mimics a real Argus usage pattern:
//
      ...
      sub = await watcher.subscribe(
        ...
        (err, events) => {
          if (err) {
            recordError(err)
            errored = true
            // Argus's createWatcher error path: unsubscribe from inside the
            // error callback.
            if (sub) {
              stats.unsubscribes++
...
async function main() {
  ...
  // Outside the repo worktree: a live Argus instance may be watching the repo,
