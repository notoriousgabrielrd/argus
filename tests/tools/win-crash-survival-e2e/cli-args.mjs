...
// installed, packaged Argus.exe. So its only source is `--exe-path` (defaulting
...
Proves that force-killing ONLY Argus's main process (a real crash, no tree-kill)
...
Usage:
  node tests/tools/win-crash-survival-e2e/run.mjs --expect <profile> [--exe-path <Argus.exe>] [options]
...
Options:
  --exe-path <path>        Installed Argus.exe to drive (default: the per-user
                           install under %LOCALAPPDATA%\\Programs\\Argus). The
...
export function parseArgs(argv) {
  ...
  const opts = {
    ...
    // "no Argus.exe found" default-resolution error.
    exePath:
...
function validate(opts, exePathFlagPresent, argv) {
  ...
  if (exePathFlagPresent && takeValue(argv, '--exe-path') === undefined) {
    ...
    errors.push(
      'No installed Argus.exe found under %LOCALAPPDATA%\\Programs — pass --exe-path <Argus.exe>'
    )
