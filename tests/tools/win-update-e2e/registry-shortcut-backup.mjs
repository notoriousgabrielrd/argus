...
/**
 * Read-only discovery of the machine's current Argus install registry state.
...
export function discoverInstallRegistryState() {
  const ps = [
    ...
    `  if ($dn -eq 'Argus' -or ($il -and $il -match '\\\\Programs\\\\orca')) {`,
    ...
    `  if ($il -and ($sn -eq 'Argus' -or $il -match 'orca')) {`,
...
/** The two directories NSIS writes Argus shortcuts into: Start Menu + Desktop. */
...
/** Read-only: list Argus *.lnk shortcuts under the given dirs (recursive). */
...
/**
 * Snapshot the shared install registry keys + Argus shortcuts before an isolated
 ...
 * Pre-existing shortcuts are copied back; test-created Argus shortcuts are removed.
...
function printMismatchWarning(manifest, expected, actual) {
  ...
  console.error('!! Your REAL Argus install pointer may be hijacked to the test directory.')
  console.error('!! The next real Argus update could install into the test location.')
...
if (process.argv[1] && path.resolve(process.argv[1]) === import.meta.filename) {
  ...
  console.log('[registry-shortcut-backup] Argus shortcuts that would be backed up:')
