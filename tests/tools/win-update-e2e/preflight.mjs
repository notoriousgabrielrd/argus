...
// it created. To avoid ever touching a user's live Argus, it REFUSES to run when
// a pre-existing Argus app process (not a daemon) is already running that it did
...
/**
 * Find running Argus APP processes (main window process), excluding daemons.
 * The daemon runs as Argus.exe too but always carries the daemon-entry.js marker
 ...
 * test dir (fatal) or is the developer's real Argus elsewhere (informational).
 */
export function findAppProcesses() {
  const command = [
    `$procs = @(Get-CimInstance Win32_Process -Filter "Name = 'Argus.exe'" -ErrorAction SilentlyContinue |`,
  ...
  // query must abort the run rather than look like "no Argus is running".
  const { stdout, stderr, code, error } = runCommandSync(command)
  if (error) {
    throw new Error(`Failed to query Argus app processes: ${error.message}`)
  }
  if (code !== 0) {
    throw new Error(`Failed to query Argus app processes (exit ${code}): ${stderr || stdout}`)
  }
  ...
  try {
    ...
    throw new Error(
      `Argus app process query returned invalid JSON: ${parseError.message}\n` +
        `stdout:\n${trimmed}\nstderr:\n${stderr}`
    )
  }
}
 ...
 * pre-existing Argus app is running (never kill a user's process) or if an Argus
 ...
 * in installDir triggers the existing-install refusal. A real Argus running or
...
export function preflight({ baselinePath, allowExistingInstall = false, installDir = null }) {
  ...
  if (isolated) {
    ...
    if (inTarget.length > 0) {
      ...
      throw new Error(
        `Refusing to run: ${inTarget.length} Argus app process(es) are running from the ` +
    ...
    if (elsewhere.length > 0) {
      warnings.push(
        `${elsewhere.length} Argus app process(es) are running from outside the isolated ` +
    ...
    throw new Error(
      `Refusing to run: ${appProcesses.length} Argus app process(es) are already ` +
  ...
  if (existingInstall && !allowExistingInstall) {
    throw new Error(
      isolated
        ...
        : `Refusing to run: an Argus install already exists at ${existingInstall}. ` +
            `This run would silently OVERWRITE it with the --from/--to versions and ` +
            `leave the --to version installed — destroying a real Argus install on a ` +
            `developer machine. Pass --allow-existing-install to proceed anyway ` +
            `(your prior build will NOT be restored), or uninstall Argus first. Clean ` +
