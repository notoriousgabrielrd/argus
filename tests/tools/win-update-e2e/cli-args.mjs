...
Installer source (version N, then N+1) — path or release tag on each side:
  --from <path>            Local argus-windows-setup.exe for the base version (N)
  --to <path>              Local argus-windows-setup.exe for the update (N+1)
...
Options:
  --install-dir <path>     Isolated-install mode: install the test build into
                           ...
                           leaving a developer's REAL Argus install untouched.
  ...
  --allow-existing-install Proceed even if an Argus install already exists. The
                           ...
                           developer's real Argus. Clean machines (CI/VM) never
...
/** Default per-user oneClick install location: %LOCALAPPDATA%\Programs\Argus. */
function defaultInstallDir() {
  const localAppData =
    process.env.LOCALAPPDATA ?? path.join(process.env.USERPROFILE ?? '', 'AppData', 'Local')
  return path.join(localAppData, 'Programs', 'Argus')
}
...
function looksLikeHarnessInstall(dir) {
  return existsSync(path.join(dir, 'Argus.exe')) && existsSync(path.join(dir, 'Uninstall Argus.exe'))
}
    ...
    if (entries.length > 0 && !looksLikeHarnessInstall(installDir)) {
      errors.push(
        `--install-dir "${installDir}" is a non-empty directory that does not look like a ` +
          `prior harness install (no Argus.exe + "Uninstall Argus.exe"). Refusing to overwrite ` +
