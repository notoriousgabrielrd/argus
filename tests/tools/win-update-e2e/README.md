...
**Windows only.** Given two Argus Windows installers (version N and N+1), this
...
- **The daemon is identified by command-line marker, never by exe name.** With
  `ELECTRON_RUN_AS_NODE` the daemon image is `Argus.exe`; a relocated Phase 1
...
| Flag                                          | Meaning                                                     |
| --------------------------------------------- | ----------------------------------------------------------- |
| `--from <path>` / `--to <path>`               | Local `argus-windows-setup.exe` for base (N) / update (N+1)  |
...
- **Pre-existing app process → hard refusal.** If an Argus _app_ process (not a
...
- **Pre-existing install → refusal unless `--allow-existing-install`.** If an
  Argus install already exists under `%LOCALAPPDATA%\Programs`, the run refuses,
...
(`%LOCALAPPDATA%\Programs\Argus`). A developer's box already has a real Argus there,
...
1. **Snapshots** the shared state (`registry-shortcut-backup.mjs`): `reg export`s
   each existing key to `.reg` files, copies the Argus `*.lnk` shortcuts, and records
...
and the Argus shortcuts — the state that steers a future update and the user-visible
...
1. **Preflight** — assert win32; warn if elevated; **refuse** to run if a
   pre-existing Argus _app_ process (not a daemon) is running that the harness
...
2. **Install N** silently (`<setup.exe> /S`) and locate `Argus.exe`.
...
| `registry-shortcut-backup.mjs` | Isolated mode: snapshot/restore the shared HKCU keys + Argus shortcuts     |
