...
//
// Argus ships a per-user oneClick NSIS installer (electron-builder defaults:
// oneClick=true, perMachine=false) named argus-windows-setup.exe. One-click
// silent mode is `<setup.exe> /S`; the app installs under
// %LOCALAPPDATA%\Programs\<dir> and the exe is Argus.exe. The install dir casing
...
export function silentInstall(setupExe, { timeoutMs = 180_000, installDir = null } = {}) {
  ...
  // On update runs the old Argus.exe already exists, so wait for the exe whose
...
/**
 * Locate the installed Argus.exe. In isolated mode (`installDir` set), the exe is
 * at a known fixed path (<installDir>\Argus.exe). Otherwise it is discovered
 ...
 * developer's REAL Argus. It additionally refuses to run against the default
  ...
  if (!allowDefaultLocation && pathsEqual(resolved, path.join(programsRoot(), PRODUCT_NAME))) {
    throw new Error(
      `Refusing to uninstall the default install location "${resolved}" — this is where a ` +
        `developer's REAL Argus lives. Isolated mode must target a separate --install-dir.`
    )
  }
