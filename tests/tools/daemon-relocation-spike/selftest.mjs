...
function makeSyntheticInventory() {
  ...
  return {
    appDir: 'C:\\App',
    unpackedRoot,
    hostExe: { name: 'Argus.exe', ...f('C:\\App\\Argus.exe') },
    runtimeData: [
...
function run() {
  ...
  for (const tier of TIERS) {
    ...
    check(`${tier}: has exe`, dests.includes('Argus.exe'))
  ...
  const modules = [
    'C:\\Users\\me\\AppData\\Local\\Programs\\orca\\Argus.exe',
    'C:\\Windows\\System32\\kernel32.dll',
    'C:\\Users\\me\\AppData\\Local\\orca-daemon-host\\Argus.exe'
  ]
  ...
  check(
    ...
    findAppDirResidentModules(['c:/a/ARGUS/x.dll'], 'C:\\A\\orca').length === 1
  )
  check(
    'relocated host has zero app-dir modules',
    findAppDirResidentModules(
      ['C:\\work\\daemon-host\\Argus.exe', 'C:\\Windows\\System32\\ntdll.dll'],
      appDir
    ).length === 0
  )
