...
> `win-unpacked` build so that a **copied `Argus.exe`** — run with
...
The daemon is `fork()`ed from the app's own Electron binary (`Argus.exe`) with
...
round-trips the nonce, the daemon's main module is the copied `Argus.exe`, and
**no** loaded module resolves under `--app-dir`.

## Tiers (defined as data in `tier-file-set.mjs`)

Every tier includes the irreducible core: `Argus.exe`, `icudtl.dat`, both V8
...
process's **main module** is the copied `Argus.exe`, not the install-dir one.
...
- Whether `TIER_MINIMAL` (no top-level DLLs) boots `Argus.exe` as node at all, or
...
ship minimal; if `Argus.exe` fails to boot without the non-GPU DLLs, fall back to
