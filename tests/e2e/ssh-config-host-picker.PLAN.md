...
secondary **Add all N to Argus** action (no re-adopt). Settings → SSH → **Import**
...
1. **Isolated HOME** — E2E already sets `HOME` to `{userDataDir}/home`. Seed config with:
   ...
   ```
2. **Unique aliases** — prefix Host entries and Argus labels with
...
| **Expect** | Dialog title **Choose from ~/.ssh/config**; body **No hosts in ~/.ssh/config**; **Add all to Argus** disabled; **Back** returns to form. |
...
| **Steps** | Open picker. |
| **Expect** | Host list `SSH config hosts` shows both aliases; subtitle `user@hostname:port`; button **Add all 2 to Argus** enabled. |
...
| | |
|--|--|
| **Setup** | Config hosts alpha + bravo. Seed Argus target with `configHost`/`label` matching alpha (via `ssh.addTarget`). |
| **Steps** | Open picker. |
| **Expect** | Alpha shows **In Argus** badge and is not clickable; bravo still selectable; **Add all 1 to Argus** (not 2). |

### P6 — Add all N to Argus imports new hosts only

| | |
|--|--|
| **Setup** | Config with 2 new hosts; no Argus targets for them. |
| **Steps** | **Add all 2 to Argus** → wait for success toast / return to form or list refresh. |
| **Expect** | Both targets exist (DOM in Settings SSH and/or listTargets); re-open picker shows **All hosts already in Argus** / both **In Argus**. |
...
| **Expect** | Deleted config host reappears as an Argus target; toast sync count ≥ 1. |
...
- **N2** Case-insensitive alias: config `Prod`, existing label `prod` → **In Argus**.
...
| P5 In Argus badge / count | `ssh-config-host-import.spec.ts` |
